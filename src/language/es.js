export const spain = {
  "Addresses": "Direcciones",
  "Settings": "Configuración",
  "Inbox": "Bandeja de entrada",
  "Interests": "Intereses",
  "Orders & Trips": "Pedidos y viajes",
  " ": " ",
  " ": " ",
  " Add New +": "أضف جديد +",
  " Add new address": " أضف عنوان جديد",
  " Add new address": " أضف عنوان جديد",
  " Change": " يتغيرون",
  " I agree that placing the order places me under Terms and Conditions & Privacy Policies":
    " أوافق على أن تقديم الطلب يضعني ضمن الشروط والأحكام وسياسات الخصوصية",
  " Loyalty Points": " نقاط الولاء",
  " Reviews": " Reseñas",
  " Show less": " Mostrar menos",
  " Take away": " يبعد",
  " Take away": " يبعد",
  " Terms & Conditions": " Términos y Condiciones",
  " from your monthly orders? This action cannot be undone.":
    " from your monthly orders? This action cannot be undone.",
  " that is used in this order as you have ordered as a guest user. Otherwise, you won’t be able to track your order in the future.":
    " que se usa en este pedido ya que lo hizo como invitado. De lo contrario, no podrá rastrear su pedido en el futuro.",
  " that is used in this order as you have ordered as guest user. Other wise you won’t be able to track your order in future.":
    " que se usa en este pedido ya que lo hizo como invitado. De lo contrario, no podrá rastrear su pedido en el futuro.",
  "%": "%",
  "% OFF": "% DE DESCUENTO",
  "% discount": "% de descuento",
  "% off": "٪ عن",
  "% off on all foods": "٪ خصم على جميع الأطعمة",
  "% off": "٪ عن",
  "% off on all foods": "٪ خصم على جميع الأطعمة",
  "&": "y",
  "(Choose Payment Method)": "(Elige el método de pago)",
  "(Due)": "(Pendiente)",
  "(Faster & secure way to pay bill)": "(Forma más rápida y segura de pagar)",
  "(Inc. VAT/TAX)": "(IVA/Imp. incl.)",
  "(Non-Veg)": "غير نباتي",
  "(Non-Veg)": "غير نباتي",
  "(Vat/Tax incl.)": "(IVA/Imp. incl.)",
  "(Veg)": "نباتي",
  "(optional)": "(اختياري)",
  "(required)": "(مطلوب)",
  "(Veg)": "نباتي",
  "(optional)": "(اختياري)",
  "(required)": "(مطلوب)",
  "* By login I Agree with all the": "* Al iniciar sesión acepto todos los",
  "* Please select an option to pay the rest of the amount":
    "* Por favor selecciona una opción para pagar el monto restante",
  "* Please select how to pay remain billing amount.":
    "* Por favor selecciona cómo pagar el monto restante.",
  ", Please keep this Order ID handy for tracking":
    ", Por favor guarda este ID de pedido para el seguimiento",
  ". You can use this ID to track your order later":
    ". Puedes usar este ID para rastrear tu pedido más tarde",
  "...Read more": "...Leer más",
  "/day": "/día",
  "/hr": "/h",
  "/km": "/km",
  "1k+ km": "1k+ km",
  "24/7 Helpline": "24/7 خط المساعدة",
  "6amMart": "6amMart",
  "6amMart is a Laravel and Flutter Framework-based multi-vendor food, grocery, eCommerce, parcel, and pharmacy delivery system. It has six modules to cover all your business functionality.":
    "6amMart عبارة عن نظام متعدد البائعين للأغذية والبقالة والتجارة الإلكترونية والطرود والصيدليات قائم على Laravel و Flutter Framework. يحتوي على ست وحدات لتغطية جميع وظائف عملك.",
  "A Module has been selected.": "Se ha seleccionado un módulo.",
  "A location where you want to pick up the parcel":
    "Un lugar donde deseas recoger el paquete",
  "A location where you want to send the parcel":
    "Un lugar a donde deseas enviar el paquete",
  "A module has been selected.": "تم اختيار الوحدة النمطية.",
  "A module must be selected to continue.": "يجب تحديد وحدة للمتابعة.",
  "A store must be selected to continue": "يجب تحديد متجر للمتابعة",
  "A to Z": "من الألف إلى الياء",
  AC: "AC",
  AD: "AD",
  "AI Assistant": "Asistente IA",
  "AUTHENTIC ASIAN CUISINE": "المطبخ الآسيوي الأصيل",
  "About Us": "معلومات عنا",
  "About us": "معلومات عنا",
  Accept: "يقبل",
  "Accept Cookies.": "Aceptar cookies.",
  "According to government regulations, prescription is mandatory for ordering some medicines.":
    "Según las regulaciones gubernamentales, la receta es obligatoria para pedir algunos medicamentos.",
  "Account Info": "Información de la cuenta",
  "Account Information": " معلومات الحساب ",
  "Account Information": " معلومات الحساب ",
  "Account has been deleted": "La cuenta ha sido eliminada",
  Action: " عمل ",
  Active: "Activo",
  "Active Order": "أمر نشط",
  "Active Since": "Activo desde",
  "Active orders": "أوامر نشطة",
  Add: "Agregar",
  "Add Address": "Agregar dirección",
  "Add Contact Info": "Agregar información de contacto",
  "Add Coupon": "Agregar cupón",
  "Add Custom Note": "Agregar nota personalizada",
  "Add Cutlery": "إضافة السكاكين",
  "Add Cutlery": "إضافة السكاكين",
  "Add Fund": "Agregar fondos",
  "Add Fund to Wallet": "إضافة أموال إلى Wallet",
  "Add Fund to Wallet": "إضافة أموال إلى Wallet",
  "Add Image": "Agregar imagen",
  "Add More Delivery Instruction": "إضافة المزيد من تعليمات التوصيل",
  "Add More Delivery Instruction": "إضافة المزيد من تعليمات التوصيل",
  "Add More Vehicle": "Agregar más vehículos",
  "Add New": "Agregar nuevo",
  "Add New +": "إضافة + جديد",
  "Add New Address": "أضف عنوان جديد",
  "Add New Provider": " إضافة موفر جديد ",
  "Add New +": "إضافة + جديد",
  "Add New Address": "أضف عنوان جديد",
  "Add New Provider": " إضافة موفر جديد ",
  "Add Ons": "Complementos",
  "Add Ons (OPTIONAL)": "الإضافات (اختياري)",
  "Add Ons (Optional)": "إضافات (اختياري)",
  "Add Ons (OPTIONAL)": "الإضافات (اختياري)",
  "Add Ons (Optional)": "إضافات (اختياري)",
  "Add Payment Method": "Agregar método de pago",
  "Add To Cart": "Agregar al carrito",
  "Add To Monthly Order": "Agregar al pedido mensual",
  "Add To Monthly Order": "Agregar al pedido mensual",
  "Add To cart": "Agregar al carrito",
  "Add a new address to get started":
    "Agrega una nueva dirección para comenzar",
  "Add at least one option to pay your order.":
    "Agrega al menos una opción para pagar tu pedido.",
  "Add fund": "أضف تمويلاً",
  "Add fund by from secured digital payment gateways":
    "أضف الأموال من بوابات الدفع الرقمية المؤمنة",
  "Add new address": "إضافة عنوان جديد",
  "Add new address +": "إضافة عنوان جديد +",
  "Add to Cart": "أضف إلى السلة",
  "Add to cart": "أضف إلى السلة",
  "Add to favorite": "أضف إلى المفضلة",
  "Add to wishlist": "أضف إلى قائمة الامنيات",
  "Add {{name}} to cart": "Añadir {{name}} al carrito",
  "Add-on": "اضافه",
  "Added to cart successfully": "Agregado al carrito con éxito",
  "Added to favorite successfully.": "تمت الإضافة إلى المفضلة بنجاح.",
  "Added to wishlist": "Added to wishlist",
  "Adding...": "Adding...",
  "Additional Charge": "Cargo adicional",
  "Additional Information": "Información adicional",
  "Additional Note": "مذكرة إضافية",
  "Additional Note": "مذكرة إضافية",
  "Additional Notes": "Notas adicionales",
  Addon: "اضافه",
  Addons: "Complementos",
  "Addons Price": "سعر الإضافات",
  Address: " تبوك ",
  "Address Form": " استمارة العنوان ",
  "Address Type": "نوع العنوان",
  "Address deleted successfully.": "Dirección eliminada con éxito.",
  "Address is required": " العنوان مطلوب ",
  Addresses: "Direcciones",
  Admin: "Administrador",
  "Admin also reward their top customers with wallet money":
    "El administrador también recompensa a sus mejores clientes con dinero en la billetera",
  "Admin charge test": "Prueba de cargo de administrador",
  "Admin will confirm your registration after review":
    "El administrador confirmará tu registro después de la revisión",
  "After completing the order, you will receive a":
    "Después de completar el pedido, recibirás un",
  "After completing the trip, you will receive a":
    "Después de completar el viaje, recibirás un",
  Agree: "يوافق على",
  "Air Conditioned": "Con aire acondicionado",
  Alerts: " تنبيهات ",
  All: "الجميع",
  "All Cart": "Todo el carrito",
  "All Items": "Todos los artículos",
  "All Products": "Todos los productos",
  "All Restaurants": "جميع المطاعم",
  "All Stores": "جميع المتاجر",
  "All Restaurants": "جميع المطاعم",
  "All Stores": "جميع المتاجر",
  "All Transaction": "Todas las transacciones",
  "All Vehicles": "Todos los vehículos",
  "All files deleted successfully": "Todos los archivos eliminados con éxito",
  "All items from": "All items from",
  "All items from": "All items from",
  "Allergic Ingredients": "Ingredientes alergénicos",
  "Allowed formats:": "Formatos permitidos:",
  "Already have an account?": "هل لديك حساب؟",
  "Also Brought Together": "También llevado juntos",
  Amount: " مقدار ",
  "Amount Due": "Amount Due",
  "Amount in Wallet": "المبلغ في المحفظة",
  "Amount to pay": "Importe a pagar",
  "An account is set up with sender’s name, phone & email to unlocking all the awesome features just for you!":
    "Se configura una cuenta con el nombre, teléfono y correo del remitente para desbloquear todas las funciones increíbles para ti.",
  "An additional": "An additional",
  "An additional $2.00 will be applied.":
    "Se aplicará un cargo adicional de $2.00.",
  Announcement: "Announcement",
  "Another provider car exit in your cart":
    "Hay un coche de otro proveedor en tu carrito",
  "Anytime With": "En Cualquier Momento Con",
  "App Store": "App Store",
  "App Vendor": "Proveedor de la app",
  Apple: "Apple",
  Applied: "Aplicado",
  "Applied filters:": "Filtros aplicados:",
  Apply: "Aplicar",
  "Apply Coupon": "Apply Coupon",
  "Apply Now": "قدم الآن",
  "Apply your Pro coupon above to claim the discount on this order.":
    "Apply your Pro coupon above to claim the discount on this order.",
  "Approx. Pickup Time:": "Hora aprox. de recogida:",
  "Are You Sure You Want To Delete ?": "هل أنت متأكد أنك تريد حذف ؟",
  "Are You Sure You Want To Logout?": "هل أنت متأكد أنك تريد تسجيل الخروج؟",
  "Are Your Sure You Want to Delete This Zone ?":
    " هل أنت متأكد أنك تريد حذف هذه المنطقة؟ ",
  "Are you sure to change the language?": "هل أنت متأكد من تغيير اللغة؟",
  "Are you sure want to reset": "¿Estás seguro que quieres restablecer?",
  "Are you sure want to reset ?": "¿Estás seguro que quieres restablecer?",
  "Are you sure you want to  delete this item?":
    "هل أنت متأكد أنك تريد حذف هذا البند؟",
  "Are you sure you want to  logout?": "هل أنت متأكد أنك تريد تسجيل الخروج؟",
  "Are you sure you want to cancel the booking?":
    "¿Estás seguro que quieres cancelar la reserva?",
  "Are you sure you want to cancel this order?":
    "هل أنت متأكد أنك تريد إلغاء هذا الطلب؟",
  "Are you sure you want to cancel your subscription? You will lose access to exclusive Pro benefits and savings.":
    "¿Seguro que quieres cancelar tu suscripción? Perderás acceso a beneficios y ahorros exclusivos de Pro.",
  "Are you sure you want to clear cart?":
    "¿Estás seguro que quieres vaciar el carrito?",
  "Are you sure you want to delete this address?":
    "هل أنت متأكد أنك تريد حذف هذا العنوان؟",
  "Are you sure you want to delete this item?":
    "¿Estás seguro que quieres eliminar este artículo?",
  "Are you sure you want to delete your account?":
    "هل انت متأكد انك تريد حذف حسابك؟",
  "Are you sure you want to logout?":
    "¿Estás seguro que quieres cerrar sesión?",
  "Are you sure you want to remove": "Are you sure you want to remove",
  "Are you sure you want to switch this order to Cash On Delivery?":
    'هل أنت متأكد أنك تريد تحويل هذا الطلب إلى "الدفع عند التسليم"؟',
  "Are you sure, you want to reset?": "هل أنت متأكد أنك تريد إعادة تعيين؟",
  Ascending: "Ascendente",
  "Ask AI Assistant": "Pregunta al Asistente IA",
  "Ask anything — orders, deals, restaurants, recommendations.":
    "Pregunta lo que quieras — pedidos, ofertas, restaurantes, recomendaciones.",
  "Attach photo": "إرفاق صورة",
  "Automatically reorder these items every month.":
    "Reordena automáticamente estos artículos cada mes.",
  Available: "Disponible",
  "Available Coupons": "Cupones disponibles",
  "Available Form": "النموذج المتاح",
  "Available Form": "النموذج المتاح",
  "Available at": "Disponible en",
  "Available on": "Disponible en",
  "Available will be": "سيكون متاحا",
  "Avoid calling phone": "تجنب الاتصال بالهاتف",
  "BEST TACOS AROUND": "أفضل تاكو حولها",
  Back: " خلف ",
  "Back to": "Volver a",
  "Back to Home": "Volver al inicio",
  "Back to Orders": "Volver a pedidos",
  "Back to chat list": "Volver a la lista de chats",
  "Back to home": "العودة إلى المنزل",
  Background: "Fondo",
  Balance: "Saldo",
  "Bank Info": "Información bancaria",
  "Be prepare to receive when they complete there first purchase":
    "Prepárate para recibir cuando completen su primera compra",
  "Become A Deliveryman": "كن موظف توصيل",
  "Become A Seller": "كن بائعا",
  "Become a Vendor owner": "Conviértete en propietario de un comercio",
  "Become a delivery man": "كن رجل توصيل",
  "Become a restaurant owner": "كن صاحب مطعم",
  "Become a store owner": "كن صاحب متجر",
  "Best Restaurants": "أفضل المطاعم",
  "Best Restaurants Near Your Location": "أفضل المطاعم بالقرب من موقعك",
  "Become a delivery man": "كن رجل توصيل",
  "Become a restaurant owner": "كن صاحب مطعم",
  "Become a store owner": "كن صاحب متجر",
  "Best Restaurants": "أفضل المطاعم",
  "Best Restaurants Near Your Location": "أفضل المطاعم بالقرب من موقعك",
  "Best Resturants": "Mejores restaurantes",
  "Best Reviewed": "أفضل مراجعة",
  "Best Reviewed Food": "أفضل طعام تمت مراجعته",
  "Best Reviewed Foods": "أفضل الأطعمة التي تمت مراجعتها",
  "Best Reviewed": "أفضل مراجعة",
  "Best Reviewed Food": "أفضل طعام تمت مراجعته",
  "Best Reviewed Foods": "أفضل الأطعمة التي تمت مراجعتها",
  "Best Reviewed Items": "Artículos mejor valorados",
  "Best Store Nearby": "Mejor tienda cercana",
  "Best reviewed Foods": "Comidas mejor valoradas",
  "Best reviewed items": "أفضل العناصر التي تمت مراجعتها",
  "Best reviewed items": "أفضل العناصر التي تمت مراجعتها",
  "Bill Details": "Detalles de la factura",
  Billing: "الفواتير",
  "Booking ID": "ID de reserva",
  Brand: "Marca",
  Brands: "العلامات التجارية",
  "Bring Change Instruction": "Instrucción para traer cambio",
  "Browse thousands of products and get them delivered today.":
    "Explora miles de productos y recíbelos hoy mismo.",
  "Business Address": "Dirección del negocio",
  "Business Cover": "Portada del negocio",
  "Business Information": " معلومات العمل ",
  "Business Information": " معلومات العمل ",
  "Business Logo": "Logo del negocio",
  "Business Module": "Módulo del negocio",
  "Business Name": "Nombre del negocio",
  "Business Plan": "Plan de negocio",
  "Business TIN": "TIN del negocio",
  "Business Zone": "Zona del negocio",
  "Business address": "Dirección del negocio",
  "Business name": "Nombre del negocio",
  "Buy 1 Get 1 Free": "Compra 1 y llévate 1 gratis",
  "Buy Now": "اشتري الآن",
  "Buy Now": "اشتري الآن",
  "By continuing, you agree to our": "Al continuar, aceptas nuestros",
  "By placing the booking you are agreed to the":
    "Al realizar la reserva aceptas los",
  "CALL US": "اتصل بنا",
  "CLOSED NOW": "مغلق الآن",
  "Call me ASAP": "الاتصال بي في اسرع وقت ممكن",
  "Campaign Duration": "Duración de la campaña",
  "Campaign Schedule :": "جدول الحملة:",
  Campaigns: "الحملات",
  Cancel: " يلغي ",
  "Cancel By": "Cancelar antes de",
  "Cancel Order": "الغاء الطلب",
  "Cancel Parcel": "Cancel Parcel",
  "Cancel Reason": "إلغاء السبب",
  "Cancel Subscription": "Cancelar suscripción",
  "Cancel Subscription?": "¿Cancelar suscripción?",
  "Cancellation Policy": "سياسة الإلغاء",
  "Cancellation Reason": "سبب الإلغاء",
  "Cancellation policy": "سياسة الإلغاء",
  "Cancelled By": "ألغى بواسطة",
  Capacity: "الاهلية",
  "Car is unavailable": "El auto no está disponible",
  "Card Number": " رقم البطاقة ",
  Cart: "عربة التسوق",
  "Cart cleared successfully": "Carrito vaciado con éxito",
  "Cart is Empty": "السلة فارغة",
  "Cart is Empty": "السلة فارغة",
  "Cart item remove successfully": "Artículo eliminado del carrito con éxito",
  "Cart updated": "Carrito actualizado",
  Carts: "Carritos",
  "Cash On Delivery": "Pago contra entrega",
  "Cash Payment": "Pago en efectivo",
  "Cash back Transactions": "Transacciones de reembolso",
  "Cash on delivery": "الدفع عند الاستلام",
  Categories: "فئات",
  Category: " فئة ",
  "Category Setup": " إعداد الفئة ",
  "Category:": "Categoría:",
  Change: "Cambiar",
  "Change Amount ($)": "Cambiar monto ($)",
  "Change Plan": "Cambiar plan",
  "Change language": "تغيير اللغة",
  "Charge Paid By": "الرسوم المدفوعة",
  "Change Plan": "Cambiar plan",
  "Change language": "تغيير اللغة",
  "Charge Paid By": "الرسوم المدفوعة",
  "Charge Pay By": "Cargo pagado por",
  Charts: " الرسوم البيانية ",
  Chat: "محادثة",
  "Chat deleted": "Chat eliminado",
  "Chat with Seller": "Chatear con el vendedor",
  "Checking theme and rtl": "Verificando tema y rtl",
  Checkout: " الدفع ",
  "Choose Delivery Instructions": "Elige instrucciones de entrega",
  "Choose Order Type": "Elige el tipo de pedido",
  "Choose Payment Method": "Elige el método de pago",
  "Choose Required Option": "Elige la opción requerida",
  "Choose Subscription Package": "Elige el paquete de suscripción",
  "Choose Your Business Plan": "Elige tu plan de negocio",
  "Choose Your Interests": "Elige tus intereses",
  "Choose Your Payment Option": "Elige tu opción de pago",
  "Choose Your Plan": "Elige tu plan",
  "Choose a Plan": "Elige un plan",
  "Choose from a wide range of cars and enjoy a smooth, reliable rental experience.":
    "Elige entre una amplia gama de coches y disfruta de una experiencia de alquiler fluida y confiable.",
  "Choose one from your existing messages, or start a new one.":
    "Elige uno de tus mensajes existentes o comienza uno nuevo.",
  "Choose preferable time when you want delivery":
    "Elige la hora preferida para la entrega",
  "Choose the Reason for Support": "Elige el motivo de soporte",
  "Choose which person will pay for the charges":
    "Elige quién pagará los cargos",
  "Choose which type of item you want to send":
    "Elige qué tipo de artículo deseas enviar",
  "Choose your delivery destination and enjoy fast, reliable parcel service.":
    "Elige tu destino de entrega y disfruta de un servicio de paquetería rápido y confiable.",
  "Chose an image max size 2mb": "Elige una imagen de máximo 2 MB",
  Clear: "واضح",
  "Clear All": "امسح الكل",
  "Clear All Filter": "مسح الكل عامل التصفية",
  "Clear Cart": "مسح السلة",
  "Clear all Filter": " مسح الكل عامل التصفية ",
  "Clear search": "Clear search",
  "Click the microphone icon in your browser's address bar":
    "Haz clic en el icono del micrófono en la barra de direcciones de tu navegador",
  "Click to upload": "Haz clic para subir",
  "Click to view": "Haz clic para ver",
  "Click to view The file": "Haz clic para ver el archivo",
  Close: "قريب",
  "Close fullscreen": "إغلاق ملء الشاشة",
  Closed: "Cerrado",
  "Closed Now": "مغلق الآن",
  Code: "شفرة",
  "Collect Cash": "جمع النقود",
  "Collect Cash From Provider": " تحصيل النقود من المزود ",
  Color: "Color",
  "Come with no sound": "تعال بدون صوت",
  Comment: "تعليق",
  Comments: "Comentarios",
  "Commission Base": "Base de comisión",
  "Common Conditions!": "¡Condiciones comunes!",
  "Communicate with our support team to get proper guidance to your quaternaries.":
    "تواصل مع فريق الدعم الخاص بنا للحصول على التوجيه المناسب لأرباعك.",
  Company: "Empresa",
  "Company / Individual Name": " اسم الشركة / الفرد ",
  "Company / Individual name is required": " اسم الشركة / الفرد مطلوب ",
  "Complete Multipurpose": "كاملة متعددة الأغراض",
  "Complete Order": "Completar pedido",
  "Complete Registration": "Completar registro",
  Completed: "Completado",
  "Completed Successfully": "Completado con éxito",
  Components: " عناصر ",
  Confirm: "Confirmar",
  "Confirm Booking": "Confirmar reserva",
  "Confirm Order": "Confirmar pedido",
  "Confirm Parcel Request": "تأكيد طلب الطرد",
  "Confirm Password": " تأكيد كلمة المرور ",
  "Confirm Password required": "مطلوب تأكيد كلمة المرور",
  "Confirm Parcel Request": "تأكيد طلب الطرد",
  "Confirm Password": " تأكيد كلمة المرور ",
  "Confirm Password required": "مطلوب تأكيد كلمة المرور",
  "Confirm Rent": "Confirmar alquiler",
  "Confirm Schedule": "Confirmar horario",
  "Confirm password is required": "Se requiere confirmar la contraseña",
  "Confirm this request ? ": " تأكيد هذا الطلب؟",
  "Confirm this request ? ": " تأكيد هذا الطلب؟",
  "Confirm this request?": "¿Confirmar esta solicitud?",
  Confirmation: "Confirmación",
  "Congratulations!": "تهانينا!",
  "Connect with our social media and other sites to keep up to date":
    "تواصل مع وسائل التواصل الاجتماعي الخاصة بنا والمواقع الأخرى للبقاء على اطلاع دائم",
  Contact: "اتصال",
  "Contact Person": " الشخص الذي يمكن الاتصال به ",
  "Contact Person Email": "Correo de la persona de contacto",
  "Contact Person Name": "اسم شخص الاتصال",
  "Contact Person Number": "رقم جهة الاتصال",
  "Contact Us": "اتصل بنا",
  "Contact Person Name": "اسم شخص الاتصال",
  "Contact Person Number": "رقم جهة الاتصال",
  "Contact Us": "اتصل بنا",
  "Contact With Vendor": "Contactar al comercio",
  "Contact us": "اتصل بنا",
  "Contact with the site owner to activate modules.":
    "تواصل مع مالك الموقع لتفعيل الوحدات.",
  Continue: "Continuar",
  "Continue Booking": "Continuar reserva",
  "Continue Delivery": "Continuar entrega",
  "Continue Shopping": "Continuar comprando",
  "Continue With": "Continuar con",
  "Continue as Guest": "Continuar como invitado",
  "Continue shopping ": "Continuar comprando ",
  "Continue to Login": "Continuar al inicio de sesión",
  "Continue with Apple": "Continuar con Apple",
  "Continue with Facebook": "Continuar con Facebook",
  "Continue with Google": "Continuar con Google",
  "Continue with facebook": "تواصل مع Facebook",
  "Continue with facebook": "تواصل مع Facebook",
  "Convert to Currency": "Convertir a moneda",
  "Convert to currency now": "تحويل إلى عملة الآن",
  "Convert your loyalty point to wallet money.":
    "Convierte tus puntos de lealtad en dinero de la billetera.",
  "Convert your loyalty points into wallet money":
    "Convierte tus puntos de lealtad en dinero de la billetera",
  Cooling: "Refrigeración",
  Copied: "تم نسخ الرمز",
  "Copied!": "¡Copiado!",
  Copy: "ينسخ",
  "Copyright ©": "حقوق النشر ©",
  "Could not copy": "Could not copy",
  "Coupon Applied": "تم تطبيق الكوبون",
  "Coupon Copied": "تم نسخ القسيمة",
  "Coupon Discount": "خصم القسيمة",
  "Coupon applied to your order.": "Cupón aplicado a tu pedido.",
  "Coupon applied to your trip.": "Coupon applied to your trip.",
  "Coupon applied to your trip.": "Coupon applied to your trip.",
  "Coupon discount": "Descuento del cupón",
  Coupons: "Cupones",
  "Cover Photo": "صورة الغلاف",
  "Cover photo is required": "صورة الغلاف مطلوبة",
  "Create Account With Sender Information":
    "Crear cuenta con información del remitente",
  "Create account with existing info.":
    "Crear cuenta con la información existente.",
  "Create account with sender info":
    "Crear cuenta con información del remitente",
  "Cuisine Restaurant": "مطعم كوزين",
  Cuisines: "المطابخ",
  "Current Location": "Ubicación actual",
  "Currently Available Foods": "الأطعمة المتوفرة حاليًا",
  "Currently Available Items": "العناصر المتاحة حاليا",
  "Currently You don’t have any message selected.":
    "Actualmente no tienes ningún mensaje seleccionado.",
  Custom: "Personalizado",
  "Custom Massage": "Mensaje personalizado",
  Cutlery: "السكاكين",
  "DELIVERY DETAILS": "تفاصيل التسليم",
  "DISCOUNT UPTO 20%": "خصم يصل إلى 20٪",
  "Daily time: ": "الوقت اليومي:",
  "Dark Mode": "الوضع الداكن",
  Dashboard: " لوحة القيادة ",
  "Date & Time": "Fecha y hora",
  Days: "Días",
  "Days Remaining": "Días restantes",
  "Days Since Joining": "Días desde la inscripción",
  "Days since Joining": "عدد الأيام منذ الانضمام",
  Default: "Predeterminado",
  Delete: "حذف",
  "Delete Account": "Eliminar cuenta",
  "Delete My Account": "احذف حسابي",
  "Delete My Account": "احذف حسابي",
  "Delete Your Account": "Eliminar tu cuenta",
  "Delete all saved files?": "¿Eliminar todos los archivos guardados?",
  "Delete chat": "Eliminar chat",
  "Delete this chat?": "¿Eliminar este chat?",
  "Delete chat": "Eliminar chat",
  "Delete this chat?": "¿Eliminar este chat?",
  "Delete your account": "Eliminar tu cuenta",
  "Delete your account ?": "¿Eliminar tu cuenta?",
  "Deleting your account will remove all your orders, addresses, wallet balance and personal data permanently.":
    "Eliminar tu cuenta eliminará permanentemente todos tus pedidos, direcciones, saldo de billetera y datos personales.",
  "Deleting…": "Eliminando…",
  "Deliver to the front door": "التوصيل إلى الباب الأمامي",
  "Deliver to the reception desk": "تسليمها إلى مكتب الاستقبال",
  Delivered: "تم التوصيل",
  "Delivering to": "يسلم ل",
  Delivery: "توصيل",
  "Delivery Address": "عنوان التسليم",
  "Delivery Addresses": "عناوين التسليم",
  "Delivery Completed": "Entrega completada",
  "Delivery Fee": "رسوم التوصيل",
  "Delivery Fee Discount": "Delivery Fee Discount",
  "Delivery Information": "معلومات التوصيل",
  "Delivery Fee": "رسوم التوصيل",
  "Delivery Fee Discount": "Delivery Fee Discount",
  "Delivery Information": "معلومات التوصيل",
  "Delivery Instruction": "Instrucción de entrega",
  "Delivery Man": "Repartidor",
  "Delivery Man Image": "صورة رجل التسليم",
  "Delivery Man Tips": "نصائح رجل التوصيل",
  "Delivery Man Type": "نوع رجل التوصيل",
  "Delivery Options": "خيارات التوصيل",
  "Delivery Time": "موعد التسليم",
  "Delivery Man Image": "صورة رجل التسليم",
  "Delivery Man Tips": "نصائح رجل التوصيل",
  "Delivery Man Type": "نوع رجل التوصيل",
  "Delivery Options": "خيارات التوصيل",
  "Delivery Time": "موعد التسليم",
  "Delivery Time is required": "Se requiere la hora de entrega",
  "Delivery Tips": "Propinas de entrega",
  "Delivery Type": "Tipo de entrega",
  "Delivery Zone": "Zona de entrega",
  "Delivery fee": "رسوم التوصيل",
  "Delivery fee benefit": "Beneficio de gastos de envío",
  "Delivery fee benefit as a Pro member":
    "Beneficio de gastos de envío como miembro Pro",
  "Delivery fee benefit as a Pro member on deliveries above {{amount}}":
    "Beneficio de gastos de envío como miembro Pro en entregas superiores a {{amount}}",
  "Delivery fee benefit as a Pro member on orders above {{amount}}":
    "Beneficio de gastos de envío como miembro Pro en pedidos superiores a {{amount}}",
  "Delivery instruction": "تعليمات التسليم",
  "Delivery man": "رجل التوصيل",
  "Delivery man doesnt assigned": "لم يتم تعيين رجل التسليم",
  "Delivery man has not been assigned": "لم يتم تعيين رجل التوصيل",
  "Delivery man image is required": "مطلوب صورة رجل التسليم",
  "Delivery man tips": "Propinas para el repartidor",
  "Deliveryman App": "تطبيق التوصيل",
  "Deliveryman App": "تطبيق التوصيل",
  "Deliveryman Application": "Solicitud de repartidor",
  "Deliveryman Registration": "تسجيل موظف التوصيل",
  "Deliveryman Registration": "تسجيل موظف التوصيل",
  "Deliveryman Type": "Tipo de repartidor",
  "Deliveryman tips": "نصائح التوصيل",
  "Deliveryman tips": "نصائح التوصيل",
  "Denied Note": "Nota de rechazo",
  Deny: "ينكر",
  Descending: "Descendente",
  Description: "وصف",
  Destination: "Destino",
  "Detail Description": "وصف التفاصيل",
  "Detailed Reviews": "مراجعات مفصلة",
  Details: "Detalles",
  Dialogs: " الحوارات ",
  "Didn’t receive the code?": "¿No recibiste el código?",
  "Digital Payment": "الدفع الرقمي",
  Disagree: "تعارض",
  Discount: " تخفيض",
  "Discount Available From": "الخصم متاح من",
  "Discount available from": "الخصم متاح من",
  "Discount on all orders": "Discount on all orders",
  "Discounted Foods": "الأطعمة المخفضة",
  "Discounted Items": "الأصناف المخفضة",
  "Discounted Stores": "Tiendas con descuento",
  Discover: "Descubre",
  "Discover Local Favorites": "Descubre los favoritos locales",
  "Discover fresh groceries through reels":
    "Descubre alimentos frescos a través de reels",
  "Discover professionals solving real problems":
    "Descubre profesionales que resuelven problemas reales",
  Distance: "مسافة",
  "Distance Wise": "Por distancia",
  "Do you want to change trip duration":
    "¿Quieres cambiar la duración del viaje?",
  "Do you want to change trip type": "¿Quieres cambiar el tipo de viaje?",
  "Do you want to login or continue as a guest?":
    "¿Quieres iniciar sesión o continuar como invitado?",
  "Do you want to use now?": "¿Quieres usarlo ahora?",
  "Don't have an account?": "ليس لديك حساب؟",
  "Don't miss out, order your favorites now":
    "No te lo pierdas, pide tus favoritos ahora",
  "Don't show this again": "No mostrar de nuevo",
  Done: "فعله",
  "Dont have a cutlery? Restaurant will provide you.":
    "لا تملك أدوات مائدة؟ سيوفر لك المطعم.",
  Download: " تحميل ",
  "Download ON": "Descargar en",
  "Download app to enjoy more!": "قم بتنزيل التطبيق للاستمتاع أكثر!",
  "Download app to enjoy more!": "قم بتنزيل التطبيق للاستمتاع أكثر!",
  "Download on": "Descargar en",
  "Download our app from google play store & app store.":
    "قم بتنزيل تطبيقنا من متجر Google play ومتجر التطبيقات.",
  "Download the Customer App": "Descarga la app para clientes",
  "Download the Driver App": "Descarga la app para conductores",
  "Download the Seller App": "Descarga la app para vendedores",
  "Download the deliveryman App": "Descarga la app para repartidores",
  "Drag & Drop": "Arrastrar y soltar",
  Driver: "Conductor",
  "Driver information is not available for this trip.":
    "La información del conductor no está disponible para este viaje.",
  "Driver will arrived soon to the pickup location":
    "El conductor llegará pronto al punto de recogida",
  Due: "Pendiente",
  "Due Amount": "Monto pendiente",
  "Due Payment": "Pago pendiente",
  "Due to payment transaction error your registration could not complete.":
    "Debido a un error en la transacción, tu registro no pudo completarse.",
  Duration: "Duración",
  "Duration type": "Tipo de duración",
  "EMAIL US": "ارسل لنا عبر البريد الإلكتروني",
  "EMAIL US": "ارسل لنا عبر البريد الإلكتروني",
  "Each file must be 2MB or smaller": "Cada archivo debe ser de 2 MB o menos",
  "Earn money on every referral": "كسب المال على كل إحالة",
  "Earn money to your wallet by completing the offer & challenged":
    "Gana dinero en tu billetera completando ofertas y retos",
  "Easiest Way to Get services": "Easiest Way to Get services",
  "Easiest way to get services": "La forma más fácil de obtener servicios",
  "Easy search to easy order & get fast delivery for your need.":
    "Búsqueda fácil para pedir fácilmente y obtener entrega rápida para lo que necesitas.",
  Edit: " يحرر ",
  "Edit Address": "Editar dirección",
  "Edit Contact Info": "Editar información de contacto",
  "Edit Info": "Editar información",
  "Edit Personal Details": "Editar datos personales",
  "Edit Profile": "Editar perfil",
  Email: " البريد الإلكتروني ",
  "Email is required": " البريد الالكتروني مطلوب ",
  "Email or phone number is required":
    "Se requiere correo o número de teléfono",
  "Email/Phone": "Correo/Teléfono",
  "Enable auto save": "Activar guardado automático",
  Enjoy: "يتمتع",
  "Enjoy Off On All Foods": "استمتع بإيقاف جميع الأطعمة",
  "Enjoy extra savings on every": "Enjoy extra savings on every",
  "Enjoy extra savings on every order with a":
    "Enjoy extra savings on every order with a",
  "Enjoy extra savings on every {{subject}} with a":
    "Disfruta de ahorros adicionales en cada {{subject}} con un",
  "Enjoy unlimited free deliveries": "Enjoy unlimited free deliveries",
  "Enter Identity Number": "Ingresa el número de identidad",
  "Enter Your Code": "Ingresa tu código",
  "Enter Your Coupon..": "أدخل قسيمتك ..",
  "Enter a promo code to save on your trip":
    "Enter a promo code to save on your trip",
  "Enter email": "Ingresa correo",
  "Enter first name": "Ingresa nombre",
  "Enter last name": "Ingresa apellido",
  "Enter phone number": "أدخل رقم الهاتف",
  "Enter the verification code (OTP) sent to":
    "Ingresa el código de verificación (OTP) enviado a",
  "Enter user name": "Ingresa nombre de usuario",
  "Enter your coupon code": "Ingresa tu código de cupón",
  "Enter your email address": "Ingresa tu correo electrónico",
  "Enter your order ID and phone number to get live updates":
    "Enter your order ID and phone number to get live updates",
  "Enter your order id": "Ingresa el ID de tu pedido",
  "Enter your promo code below.": "Enter your promo code below.",
  "Enter your trip ID and phone number to get live updates":
    "Enter your trip ID and phone number to get live updates",
  "Enter your trip id": "Ingresa el ID de tu viaje",
  "Essential medicines & health products":
    "Medicamentos esenciales y productos de salud",
  "Est. Delivery Time": "Tiempo est. de entrega",
  Estimated: "Estimado",
  "Estimated delivery:": "Entrega estimada:",
  "Ex: 8+ Character": "Ej: 8+ caracteres",
  "Ex: Please provide good conditioned vehicle":
    "Ej: Por favor proporciona un vehículo en buenas condiciones",
  "Exclusive Deals": "Ofertas exclusivas",
  "Exclusive Pro-only deals": "Exclusive Pro-only deals",
  "Exclusive collection for everyone": "مجموعة حصرية للجميع",
  "Exclusive coupon on order": "Exclusive coupon on order",
  "Exclusive coupon unlocked for Pro members.":
    "Exclusive coupon unlocked for Pro members.",
  "Experience smooth, secure, and dependable delivery from pickup to drop-off.":
    "Disfruta de una entrega fluida, segura y confiable desde la recogida hasta la entrega.",
  "Experience the future of mobility. Book fast, affordable rides in just a few taps with our executive-level service and reliable driver network.":
    "Vive el futuro de la movilidad. Reserva viajes rápidos y asequibles en pocos toques con nuestro servicio ejecutivo y una red de conductores confiables.",
  "Expert at Work": "Expertos en Acción",
  "Expired on": "انتهت يوم",
  "Expires On": "Caduca el",
  Explore: "Explorar",
  "Explore Categories": "Explorar categorías",
  "Explore Items": "استكشاف العناصر",
  "Explore Now": "استكشف الآن",
  "Explore Items": "استكشاف العناصر",
  "Explore Now": "استكشف الآن",
  "Explore Pharmacy": "Explorar farmacia",
  "Explore Restaurants": "Explorar restaurantes",
  "Explore Stores": "Explorar tiendas",
  "Explore the Trusted and Trendsetting Brands":
    "Explora las marcas confiables y de moda",
  "Express Delivery": "Entrega express",
  "Extra Packaging": "Empaquetado extra",
  "Extra on Every Add Fund": "Extra en cada recarga de fondos",
  "FIND BEST RESTAURANTS NEAR YOU": "اعثر على أفضل المطاعم بالقرب منك",
  "FIND YOUR HAPPINESS": "اكتشف سعادتك",
  Facebook: "Facebook",
  "Failed to cancel subscription": "No se pudo cancelar la suscripción",
  "Failed to copy code": "Error al copiar el código",
  "Failed to delete chat": "No se pudo eliminar el chat",
  "Failed to delete chat": "No se pudo eliminar el chat",
  "Failed to process offline payment": "Error al procesar el pago offline",
  "Failed to remove monthly order": "Failed to remove monthly order",
  "Failed to send message": "No se pudo enviar el mensaje",
  "Fast Home Delivery": "توصيل سريع للمنزل",
  "Fast Name": "اسم سريع",
  "Faster & secure way to pay bill": "طريقة أسرع وآمنة لدفع الفاتورة",
  "Faster and safer way to send money":
    "Forma más rápida y segura de enviar dinero",
  Favorites: "المفضلة",
  Favourite: "Favorito",
  Featured: "Destacado",
  "Featured Restaurants": "Restaurantes destacados",
  "Featured Store": "Tienda destacada",
  "Featured Stores": "Featured Stores",
  "Featured categories": "فئات مميزة",
  "Features Catagories": "فئات الميزات",
  "Featured Store": "Tienda destacada",
  "Featured Stores": "Featured Stores",
  "Featured categories": "فئات مميزة",
  "Features Catagories": "فئات الميزات",
  "File Upload": "Subir archivo",
  "File is required*": " الملف مطلوب *",
  "File is required*": " الملف مطلوب *",
  "File must be less than 1MB": "El archivo debe ser menor a 1 MB",
  "File too large (max 20MB)": "Archivo demasiado grande (máx. 20 MB)",
  "File type not supported": "Tipo de archivo no compatible",
  "File upload": "تحميل الملف",
  "File upload": "تحميل الملف",
  "Files saved successfully": "Archivos guardados con éxito",
  "Fill details": "Completar datos",
  Filter: " منقي ",
  "Filter By": "مصنف بواسطة",
  "Filter Search Results": "تصفية نتائج البحث",
  "Filter Your Search Results": "تصفية نتائج البحث الخاصة بك",
  "Filter by :": "Filtrar por :",
  "Filter your order": " قم بتصفية طلبك ",
  "Filter your order": " قم بتصفية طلبك ",
  "Filter your search results": "Filtra los resultados de tu búsqueda",
  "Find Free Delivery Food": "Encuentra comida con entrega gratis",
  "Find Free Delivery Grocery": "Encuentra supermercado con entrega gratis",
  "Find Free Delivery Medicine": "Encuentra medicamentos con entrega gratis",
  "Find Free Delivery Products": "Encuentra productos con entrega gratis",
  "Find Free Delivery Food": "Encuentra comida con entrega gratis",
  "Find Free Delivery Grocery": "Encuentra supermercado con entrega gratis",
  "Find Free Delivery Medicine": "Encuentra medicamentos con entrega gratis",
  "Find Free Delivery Products": "Encuentra productos con entrega gratis",
  "Find Your Flavour": "Encuentra tu sabor",
  "Find grocery stores near me": "Encuentra tiendas de comestibles cerca de mí",
  "Find pharmacies near me": "Encuentra farmacias cerca de mí",
  "Find restaurants near me": "Encuentra restaurantes cerca de mí",
  "Find shops near me": "Encuentra tiendas cerca de mí",
  "Find stores near me": "Encuentra tiendas cerca de mí",
  "Find us here": "اعثر علينا هنا",
  "First Name": " الاسم الاول ",
  "Find grocery stores near me": "Encuentra tiendas de comestibles cerca de mí",
  "Find pharmacies near me": "Encuentra farmacias cerca de mí",
  "Find restaurants near me": "Encuentra restaurantes cerca de mí",
  "Find shops near me": "Encuentra tiendas cerca de mí",
  "Find stores near me": "Encuentra tiendas cerca de mí",
  "Find us here": "اعثر علينا هنا",
  "First Name": " الاسم الاول ",
  "First name": "Nombre",
  "First name is required": " الإسم الأول مطلوب ",
  "First name is required": " الإسم الأول مطلوب ",
  "Flash Sale": "Oferta flash",
  Floor: "أرضية",
  "Floor no.": "Piso n.º",
  "Floor:": "Piso:",
  "Follow us on": "اتبعنا",
  Food: "Comida",
  "Food Campaigns": "الحملات الغذائية",
  "Food is on the way": "الطعام في الطريق",
  "Food not found": "Food not found",
  Foods: "Comidas",
  "For demo purpose use otp 123456": "Para la demostración usa el OTP 123456",
  "For the love of delicious food.": "من أجل حب الطعام اللذيذ.",
  "For the love of delicious food.": "من أجل حب الطعام اللذيذ.",
  "Forget password?": "¿Olvidaste la contraseña?",
  "Forgot Password": "هل نسيت كلمة السر",
  "Forgot Password": "هل نسيت كلمة السر",
  "Forgot Password ?": "¿Olvidaste la contraseña?",
  "Forgot password?": "هل نسيت كلمة السر؟",
  "Forgot password?": "هل نسيت كلمة السر؟",
  "Forgot your password": "Olvidaste tu contraseña",
  "Form With Formik": " شكل مع فورميك ",
  "Form with formik": "شكل مع فورميك ",
  Found: "Encontrado",
  Free: "حر",
  "Free Delivery": "توصيل مجاني",
  "Free Trial": "Prueba gratis",
  "Free delivery": "توصيل مجاني",
  "Free delivery as a Pro member": "Envío gratis como miembro Pro",
  "Free delivery as a Pro member on deliveries above {{amount}}":
    "Envío gratis como miembro Pro en entregas superiores a {{amount}}",
  "Free delivery as a Pro member on orders above {{amount}}":
    "Envío gratis como miembro Pro en pedidos superiores a {{amount}}",
  "Free delivery on eligible orders": "Free delivery on eligible orders",
  FreeLancer: "مستقل",
  "Frequently Asked Questions": "Preguntas frecuentes",
  "Fresh Finds": "Novedades frescas",
  "Fresh Item that deserve to eat": "Artículos frescos que merecen comerse",
  "Fresh deals on everyday essentials, order now":
    "Ofertas frescas en artículos esenciales, pide ahora",
  "Fresh food that deserve to eat": "طعام طازج يستحق الأكل",
  "Freshly added, just for you.": "Recién agregado, solo para ti.",
  "From You": "De ti",
  Fullscreen: "تكبير الشاشة",
  "GET IT ON": "DISPONIBLE EN",
  "General Information": " معلومات عامة ",
  "General Information": " معلومات عامة ",
  "Generic Name": "Nombre genérico",
  Get: "حصل على",
  "Get 10% off on all orders": "Get 10% off on all orders",
  "Get Groceries Up to {{discount}} Off?":
    "¡Compra comestibles con hasta {{discount}} de descuento!",
  "Get Launch Up to {{discount}} Off?":
    "¡Consigue comida con hasta {{discount}} de descuento!",
  "Get OTP": "Obtener OTP",
  "Get OTP ": "Obtener OTP ",
  "Get Your Desired High Quality Products Here":
    "احصل على منتجاتك المرغوبة عالية الجودة من هنا",
  "Get fastest order from your nearby pharmacies.":
    "Recibe tu pedido más rápido de la farmacia más cercana.",
  "Get fastest order from your nearby restaurant":
    "Recibe tu pedido más rápido del restaurante más cercano",
  "Get fastest order from your nearby store":
    "Recibe tu pedido más rápido de la tienda más cercana",
  "Get it on": "Disponible en",
  "Get personalized food recommendations.":
    "Obtén recomendaciones de comida personalizadas.",
  "Get ready for a special welcome gift, enjoy a special discount on your first order within":
    "Prepárate para un regalo de bienvenida especial, disfruta de un descuento en tu primer pedido dentro de",
  "Get your car rental service with":
    "Obtén tu servicio de alquiler de autos con",
  "Get your groceries items delivered in less than an hour":
    "احصل على سلع البقالة الخاصة بك في أقل من ساعة",
  "Get your medicine as prescribed.": "Get your medicine as prescribed.",
  "Get your recent food from the restaurant you recently ordered":
    "Obtén tu comida reciente del restaurante donde pediste recientemente",
  "Get your recent food from the restaurant you recently visited":
    "احصل على طعامك الأخير من المطعم الذي زرته مؤخرًا",
  "Get your recent medicine from the store you recently ordered":
    "Obtén tus medicamentos recientes de la tienda donde compraste recientemente",
  "Get your recent medicine from the store you recently visited":
    "احصل على أدويتك الحديثة من المتجر الذي زرته مؤخرًا",
  "Get your recent purchase from the shop you recently ordered":
    "Obtén tu compra reciente de la tienda donde compraste recientemente",
  "Get your recent purchase from the shop you recently visited":
    "احصل على آخر مشترياتك من المتجر الذي زرته مؤخرًا",
  "Getting Order List...": "الحصول على قائمة الطلبات ...",
  "Give Review": "أعط مراجعة",
  "Give a review": "قدِّم مراجعة",
  "Give a valid phone number to continue sign-in":
    "Proporciona un número de teléfono válido para continuar",
  "Give us your valuable review": "Danos tu valiosa reseña",
  "Go Back": "Volver",
  "Go Back Home": " ارجع الى البيت ",
  Google: "Google",
  "Google Play": "Google Play",
  "Got It": "Entendido",
  "Got it": "Entendido",
  "Grab The Offer Before End The Time.":
    "Aprovecha la oferta antes de que termine.",
  "Grand Total Amount": "Monto total general",
  "Grid view": "Vista de cuadrícula",
  Groceries: "Comestibles",
  Grocery: "Supermercado",
  Guest: "Invitado",
  Halal: "Halal",
  "Has  paid by your wallet.": "Se ha pagado con tu billetera.",
  "Have a Coupon?": "هل لديك قسيمة؟",
  "Have a Coupon?": "هل لديك قسيمة؟",
  "Have you received your parcel?": "¿Has recibido tu paquete?",
  "Health Highlights": "Destacados de salud",
  "Help & Support": "ساعد لدعم",
  "Health Highlights": "Destacados de salud",
  "Help & Support": "ساعد لدعم",
  "Hey there welcome to": "Hola, bienvenido a",
  "Hey there welcome to ": "Hola, bienvenido a ",
  "Hi! I'm your assistant. Ask me about orders, restaurants, or promos.":
    "¡Hola! Soy tu asistente. Pregúntame sobre pedidos, restaurantes u ofertas.",
  "Hide Reply": "Ocultar respuesta",
  "High to Low": "De mayor a menor",
  "Highlights for you": "Destacados para ti",
  History: "تاريخ",
  Home: "مسكن",
  "Home Delivery": "توصيل منزلي",
  Hourly: "Por hora",
  "Hourly Trip": "Viaje por hora",
  House: "منزل",
  "House no.": "رقم المنزل.",
  "House number": "Número de casa",
  "How can I help today?": "¿Cómo puedo ayudarte hoy?",
  "How does parcel delivery work?": "¿Cómo funciona la entrega de paquetes?",
  "How does ride booking work?": "¿Cómo funciona la reserva de viajes?",
  "How can I help today?": "¿Cómo puedo ayudarte hoy?",
  "How does parcel delivery work?": "¿Cómo funciona la entrega de paquetes?",
  "How does ride booking work?": "¿Cómo funciona la reserva de viajes?",
  "How to enable:": "Cómo activar:",
  "How was your Trip": "¿Cómo fue tu viaje?",
  "How was your experience with your last order ?":
    "¿Cómo fue tu experiencia con tu último pedido?",
  "How you it works?": "¿Cómo funciona?",
  "However, the maximum cashback amount is":
    "Sin embargo, el monto máximo de reembolso es",
  "I agree that placing the order places me under":
    "أوافق على أن وضع الطلب يضعني تحت",
  "I agree that placing the order places me under Terms and Conditions & Privacy Policies":
    "أوافق على أن تقديم الطلب يضعني ضمن الشروط والأحكام وسياسات الخصوصية",
  "I agree with the terms & conditions": "أوافق على الشروط والأحكام",
  "I will wait until it is restocked": "سأنتظر حتى يتم إعادة تخزينه",
  "I'm a Customer": "Soy un cliente",
  "I'm a Rider": "Soy un repartidor",
  "I'm a Seller": "Soy un vendedor",
  "Identity Image": "صورة الهوية",
  "Identity Image": "صورة الهوية",
  "Identity Info": "Información de identidad",
  "Identity Number": " رقم الهوية ",
  "Identity Type": "نوع الهوية",
  "Identity number Is A Required Field": "رقم الهوية هو حقل مطلوب",
  "If Any Items Is Not Available": "Si algún artículo no está disponible",
  "Identity Number": " رقم الهوية ",
  "Identity Type": "نوع الهوية",
  "Identity number Is A Required Field": "رقم الهوية هو حقل مطلوب",
  "If Any Items Is Not Available": "Si algún artículo no está disponible",
  "If Any Product is not available": "Si algún producto no está disponible",
  "If Any product is not available": "إذا كان أي منتج غير متوفر",
  "If any item is unavailable, it may be removed":
    "Si algún artículo no está disponible, puede ser eliminado",
  "If available, your order will come with cutlery.":
    "Si está disponible, tu pedido vendrá con cubiertos.",
  "If product is not available": "إذا كان المنتج غير متوفر",
  "If you accidentally provided incorrect payment information, you can edit the details in the order details section while the order is still pending.":
    "Si proporcionaste por error información de pago incorrecta, puedes editar los detalles en la sección de detalles del pedido mientras el pedido aún esté pendiente.",
  "If you cancel, your parcel will be back to you when rider will be available. You will have to pay a return fee to your delivery man.":
    "Si cancelas, tu paquete te será devuelto cuando el repartidor esté disponible. Deberás pagar una tarifa de devolución al repartidor.",
  "If you log in, your order history will be saved. However, if you continue as a guest, you won't be able to see your order history after completing your order.":
    "Si inicias sesión, tu historial de pedidos se guardará. Sin embargo, si continúas como invitado, no podrás ver tu historial de pedidos después de completar tu pedido.",
  "If you want to add fund to your wallet then click add fund button":
    "Si quieres agregar fondos a tu billetera, haz clic en el botón de agregar fondos",
  "If you want to change module type, it will reset your cart and other related cache data.":
    "Si quieres cambiar el tipo de módulo, se restablecerá tu carrito y los datos en caché relacionados.",
  "If you want to change store type, it will reset your cart and other related cache data.":
    "Si quieres cambiar el tipo de tienda, se restablecerá tu carrito y los datos en caché relacionados.",
  "If you're checking out": "Si estás finalizando la compra",
  "Image Format - JPG, Pong, ZPEG, GIF Image Size - Maximum Size 2Mb Image Ratio - 1: 1":
    " تنسيق الصورة - JPG و Pong و ZPEG و GIF حجم الصورة - أقصى حجم 2 ميجا بايت نسبة الصورة - 1: 1",
  "Image format - jpg, png, jpeg, gif Image Size - maximum size 2 MB Image Ratio - 1:1":
    "Formato de imagen - jpg, png, jpeg, gif Tamaño máximo 2 MB Proporción 1:1",
  "Image size": "Tamaño de imagen",
  "Image size must be less than 1MB":
    "El tamaño de la imagen debe ser menor a 1 MB",
  In: "En",
  "In Stock": "في الأوراق المالية",
  Inbox: "Bandeja de entrada",
  Included: "متضمن",
  Incomplete: "Incompleto",
  Info: "Información",
  "Information Details": " تفاصيل المعلومات ",
  "Insert amount if you need deliveryman to bring":
    "Ingresa el monto si necesitas que el repartidor traiga cambio",
  "Insert delivery location": "Ingresa la ubicación de entrega",
  "Instant Delivery": "Entrega instantánea",
  "Instant Same Day Delivery": "توصيل فوري في نفس اليوم",
  Instructions: "Instrucciones",
  "Internal server error": "Error interno del servidor",
  "Invite and share your code to your friends & family members":
    "Invita y comparte tu código con tus amigos y familiares",
  "Is It You?": "¿Eres tú?",
  "It Will Add To Your Balance When The Order Is Delivered":
    "ستضيف إلى رصيدك عند تسليم الطلب",
  "It looks like the email you entered has already been used and has an existing account.":
    "Parece que el correo que ingresaste ya está en uso y tiene una cuenta existente.",
  "It will add to your balance when the order is delivered.":
    "ستضيف إلى رصيدك عند تسليم الطلب.",
  "Item Details": "Detalles del artículo",
  "Item Discount": "Descuento del artículo",
  "Item Price": "سعر السلعة",
  "Item That You’ll Love": "العنصر الذي ستحبه",
  "Item added to cart": "تمت إضافة العنصر إلى سلة التسوق",
  "Item added to cart successfully": "Item added to cart successfully",
  "Item not found": "Item not found",
  "Item updated successfully": "تم تحديث العنصر بنجاح",
  Items: "Artículos",
  "Items Price": "سعر الأصناف",
  "Items Total": "Total de artículos",
  "Items You Will Love": "Artículos que te encantarán",
  "Items not found!": "لم يتم العثور على العناصر!",
  "Items not found!": "لم يتم العثور على العناصر!",
  "I’ll Pick It Up MySelf": "Lo recogeré yo mismo",
  "JPG, JPEG, PNG ,WEBP Less Than 1MB Ratio 1:1":
    "JPG, JPEG, PNG, WEBP menor a 1 MB, proporción 1:1",
  "JPG, JPEG, PNG Less Than 1MB (Ratio 1:1)":
    "JPG, JPEG, PNG menor a 1 MB (proporción 1:1)",
  "JPG, JPEG, PNG Less Than 1MB (Ratio 2:1)":
    "JPG, JPEG, PNG menor a 1 MB (proporción 2:1)",
  "JPG, JPEG, PNG Less Than 1MB Ratio 1:1":
    "JPG, JPEG, PNG menor a 1 MB, proporción 1:1",
  "JPG, JPEG, PNG, WEBP Less Than 1MB Ratio 1:1, Max 6 Images":
    "JPG, JPEG, PNG, WEBP menor a 1 MB, proporción 1:1, máx. 6 imágenes",
  "JPG, JPEG, PNG,WEBP  Less Than 1MB (Ratio 2:1)":
    "JPG, JPEG, PNG, WEBP menor a 1 MB (proporción 2:1)",
  "JPG, JPEG, PNG,WEBP Less Than 1MB (Ratio 1:1)":
    "JPG, JPEG, PNG, WEBP menor a 1 MB (proporción 1:1)",
  "JPG, JPEG, PNG,WEBP Less Than 1MB (Ratio 2:1)":
    "JPG, JPEG, PNG, WEBP menor a 1 MB (proporción 2:1)",
  Join: "انضم",
  "Join Us!": "انضم إلينا!",
  "Join as Rider": "Unirse como Conductor",
  "Join as delivery man on": "انضم كرجل توصيل في",
  "Join our online marketplace revolution and boost your income.":
    "انضم إلى ثورة سوقنا على الإنترنت وعزز دخلك.",
  "Just For You": "فقط لك",
  "Just one step away! This will help make your profile more personalized.":
    "¡A solo un paso! Esto ayudará a personalizar más tu perfil.",
  "Keep Subscription": "Mantener suscripción",
  "Label As": "Etiquetar como",
  "Label Name(Optional)": "Nombre de etiqueta (opcional)",
  Language: "لغة",
  "Language Changed Successfully": "تم تغيير اللغة بنجاح",
  "Language Changed Successfully.": "Idioma cambiado con éxito.",
  "Language changed": "Idioma cambiado",
  "Language has been changed": "تم تغيير اللغة",
  "Last Name": " الكنية ",
  "Language has been changed": "تم تغيير اللغة",
  "Last Name": " الكنية ",
  "Last Visited Place": "Último lugar visitado",
  "Last name": "Apellido",
  "Last name is required": " إسم العائلة مطلوب ",
  "Last name is required": " إسم العائلة مطلوب ",
  "Last name required": "Se requiere apellido",
  Latest: "Más reciente",
  Latitude: "خط العرض",
  "Latitude is required": "Se requiere la latitud",
  "Leave a message": "اترك رسالة",
  "Let Google help apps determine location This means sending anonymous location data to Google, even when no apps are running.":
    " دع Google تساعد التطبيقات في تحديد الموقع. هذا يعني إرسال بيانات موقع مجهولة إلى Google ، حتى في حالة عدم تشغيل أي تطبيقات.",
  "Lets Connect !": "لنتواصل!",
  "Let’s Start Earning with": "فلنبدأ في كسب المال باستخدام",
  "License No": "N.º de licencia",
  "Light Mode": "وضع الضوء",
  "Limited time offer on top products, grab yours now":
    "Oferta por tiempo limitado en los mejores productos, consigue el tuyo",
  "List view": "Vista de lista",
  "Loading Apple Login...": "Cargando inicio de sesión con Apple...",
  "Loading...": "تحميل...",
  "Loading…": "Loading…",
  "Locate me": "تحديد موقع لي",
  Location: "موقع",
  "Location From Map": "Ubicación desde el mapa",
  "Location is required.": "الموقع مطلوب.",
  "Location is required.": "الموقع مطلوب.",
  "Location set successfully": "Ubicación establecida con éxito",
  "Log Out": "Cerrar sesión",
  "Logged in successfully": "تم تسجيل الدخول بنجاح",
  "Logged in successfully": "تم تسجيل الدخول بنجاح",
  "Logged out successfully": "Sesión cerrada con éxito",
  Login: " تسجيل الدخول ",
  "Login Information": "معلومات تسجيل الدخول",
  "Login With Email/Phone": "Iniciar sesión con correo/teléfono",
  "Login With OTP": "Iniciar sesión con OTP",
  "Login here": "تسجيل الدخول من هنا",
  "Login here": "تسجيل الدخول من هنا",
  "Login or Signup": "Iniciar sesión o registrarse",
  "Login required.": "Inicio de sesión requerido.",
  "Login with": "تسجيل الدخول مع",
  "Login with": "تسجيل الدخول مع",
  "Login with OTP": "Iniciar sesión con OTP",
  "Login with Password": "Iniciar sesión con contraseña",
  "Login/Signup": "Iniciar sesión/Registrarse",
  Logo: "شعار",
  "Logo is required": "الشعار مطلوب",
  Logout: " تسجيل خروج ",
  Longitude: "خط الطول",
  "Longitude is required": "Se requiere la longitud",
  "Lorem ipsum dolor sit amet, consectetur adipiscing elit.":
    "العميل مهم جدا ، العميل سيتبعه.",
  "Low to High": "De menor a mayor",
  "Loyalty Points": "Puntos de fidelidad",
  "Loyalty Points !": "نقاط الولاء !",
  "Loyalty Points": "Puntos de fidelidad",
  "Loyalty Points !": "نقاط الولاء !",
  "Loyalty Points Transaction": "Transacción de puntos de lealtad",
  "Loyalty point": "Punto de lealtad",
  Mails: " رسائل",
  "Make sure to remember your ": "Asegúrate de recordar tu ",
  "Manage your payment manually": "Gestiona tu pago manualmente",
  "Map View": "Vista de mapa",
  "Massage to ": "Mensaje a ",
  Max: "أقصى",
  "Max 5 prescriptions reached": "Se alcanzó el máximo de 5 recetas",
  "Max Delivery Time": "Tiempo máx. de entrega",
  "Max Discount": "ماكس الخصم",
  "Max Discount": "ماكس الخصم",
  "Max Discount ": "Descuento máx. ",
  "Max Pickup Time": "Tiempo máx. de recogida",
  "Max order": "Pedido máximo",
  "Max product": "Producto máximo",
  "Max size 2MB": "Tamaño máx. 2 MB",
  "Maximum 3 images can be send at a time.":
    "يمكن إرسال 3 صور كحد أقصى في المرة الواحدة.",
  "Maximum Delivery Time": "وقت التسليم الأقصى",
  "Maximum Pickup Time": "Tiempo máximo de recogida",
  "Maximum discount": "Descuento máximo",
  "Maximum file size:": "Tamaño máximo del archivo:",
  "Maybe Later": "Quizá más tarde",
  Medicine: "Medicamento",
  Medicines: "Medicamentos",
  Messages: "Mensajes",
  "Microphone Access Required": "Acceso al micrófono requerido",
  "Microphone access has been denied. To use voice search, please enable microphone permissions in your browser settings.":
    "Se ha denegado el acceso al micrófono. Para usar la búsqueda por voz, activa los permisos del micrófono en la configuración de tu navegador.",
  Min: "دقيقة",
  "Min Delivery Time": "Tiempo mín. de entrega",
  "Min Order": "Pedido mín.",
  "Min Pickup Time": "Tiempo mín. de recogida",
  "Min Spent": "Mín. gastado",
  "Min purchase": "الحد الأدنى للشراء",
  "Minimum 200 points required to convert into currency":
    "Se requieren mínimo 200 puntos para convertir a moneda",
  "Minimum 8 characters": "Mínimo 8 caracteres",
  "Minimum Delivery Time": "الحد الأدنى من وقت التسليم",
  "Minimum Delivery Time": "الحد الأدنى من وقت التسليم",
  "Minimum Order": "Pedido mínimo",
  "Minimum Order Value": "الحد الأدنى لقيمة الطلب",
  "Minimum Order Value": "الحد الأدنى لقيمة الطلب",
  "Minimum Pickup Time": "Tiempo mínimo de recogida",
  "Minimum Purchase Value": "Valor mínimo de compra",
  "Minimum order": "Pedido mínimo",
  "Minimum order": "Pedido mínimo",
  "Minimum purchase": "Compra mínima",
  "Minimum purchase value": "الحد الأدنى لقيمة الشراء",
  "Minimum purchase value": "الحد الأدنى لقيمة الشراء",
  "Mobile app": "App móvil",
  "Module is required": "Se requiere el módulo",
  Modules: "Módulos",
  "Modules are": "Los módulos son",
  "Monthly Cart List": "Lista de carrito mensual",
  "Monthly Reorder Policy": "Política de pedido mensual",
  "Monthly order removed": "Monthly order removed",
  "Monthly Cart List": "Lista de carrito mensual",
  "Monthly Reorder Policy": "Política de pedido mensual",
  "Monthly order removed": "Monthly order removed",
  "More From Same Store": "Más de la misma tienda",
  "More From This Store!": "المزيد من هذا المتجر!",
  "More from Store": "المزيد من المتجر",
  "More products need to apply the coupon":
    "تحتاج المزيد من المنتجات لتطبيق القسيمة",
  "Most Popular": "Más populares",
  "Most Popular Items": "Artículos más populares",
  "Most Popular Products": "المنتجات الأكثر شهرة",
  "Most Popular Products": "المنتجات الأكثر شهرة",
  "Most Tipped": "Con más propinas",
  "Most Used": "Más usado",
  "Most trusted vehicles chosen by thousands of users.":
    "Los vehículos más confiables elegidos por miles de usuarios.",
  "Move the pin to select your location":
    "Move the pin to select your location",
  "Multistep Form With Steppers": " نموذج متعدد الخطوات مع ستيبرز ",
  "Must be a valid email": " يجب أن يكون بريدًا إلكترونيًا صالحًا ",
  "Must be a valid email or phone number":
    "Debe ser un correo o teléfono válido",
  "My Account": " حسابي",
  "My Addresses": "Mis direcciones",
  "My Orders": "Mis pedidos",
  "My Orders": "Mis pedidos",
  "My Payment Info": "Mi información de pago",
  "My Subscription": "Mi suscripción",
  "My Subscription": "Mi suscripción",
  "My Trips": "Mis viajes",
  "My Wallet": "محفظتى",
  "My account": "حسابي",
  "My Wallet": "محفظتى",
  "My account": "حسابي",
  "My address": "Mi dirección",
  "N/A": "N/D",
  Name: " اسم ",
  "Name is required": "مطلوب اسم",
  "Name on card": "الاسم على البطاقة",
  "Near Me": "Near Me",
  Nearby: "مجاور",
  "Need Any help?": "هل تحتاج الى مساعدة؟",
  "Need Extra Packaging?": "¿Necesitas embalaje adicional?",
  "Need to order same medicine? don’t worry just click here":
    "هل تحتاج إلى طلب نفس الدواء؟ لا تقلق ، فقط انقر هنا",
  New: "Nuevo",
  "New Arrival": "قادم جديد",
  "New Arrival Restaurants": "Restaurantes recién llegados",
  "New Arrival Stores": "متاجر وصول جديدة",
  "New Arrival Stores": "متاجر وصول جديدة",
  "New Arrivals": "Recién llegados",
  "New On": "جديد",
  "New On StackFood": "جديد على ",
  "New chat": "Nuevo chat",
  "New On": "جديد",
  "New On StackFood": "جديد على ",
  "New chat": "Nuevo chat",
  "New delivery address selected.": "Nueva dirección de entrega seleccionada.",
  "New location has been set.": "تم تعيين موقع جديد.",
  "New location has been set.": "تم تعيين موقع جديد.",
  "New on": "Nuevo en",
  "New to {{name}}?": "New to {{name}}?",
  "New to {{name}}?": "New to {{name}}?",
  "Newly Arrived": "Recién llegado",
  "Newly Joined": "Recién unido",
  Next: " التالي ",
  "Next coupon": "Siguiente cupón",
  Nid: "لا",
  No: "لا",
  "No Address Found": "No se encontró dirección",
  "No Confirm Password provided.":
    "No se proporcionó confirmación de contraseña.",
  "No Contact Info Added": "No se agregó información de contacto",
  "No Coupon Found": "لم يتم العثور على قسيمة",
  "No Cuisine Restaurant Found": "لم يتم العثور على مطعم للمأكولات",
  "No Data Found": "لاتوجد بيانات",
  "No Favourite Food Found": "لم يتم العثور على طعام مفضل",
  "No Coupon Found": "لم يتم العثور على قسيمة",
  "No Cuisine Restaurant Found": "لم يتم العثور على مطعم للمأكولات",
  "No Data Found": "لاتوجد بيانات",
  "No Favourite Food Found": "لم يتم العثور على طعام مفضل",
  "No Favourite Items Found": "No se encontraron artículos favoritos",
  "No Favourite Restaurant Found": "لم يتم العثور على مطعم مفضل",
  "No Favourite Restaurant Found": "لم يتم العثور على مطعم مفضل",
  "No Favourite Stores Found": "No se encontraron tiendas favoritas",
  "No Flash Sales Product Found!":
    "¡No se encontraron productos de oferta flash!",
  "No Orders Found": "No se encontraron pedidos",
  "No Redirect Url found": "No se encontró URL de redirección",
  "No address found": "No se encontró dirección",
  "No announcement available.": "No announcement available.",
  "No announcement available.": "No announcement available.",
  "No available coupons.": "No hay cupones disponibles.",
  "No benefits available for this module":
    "No hay beneficios disponibles para este módulo",
  "No data found": "No se encontraron datos",
  "No delivery man assigned": "No hay repartidor asignado",
  "No discounted items found": "No se encontraron artículos con descuento",
  "No food found": "لم يتم العثور على طعام",
  "No discounted items found": "No se encontraron artículos con descuento",
  "No food found": "لم يتم العثور على طعام",
  "No foods found": "No se encontraron comidas",
  "No images in media library": "No hay imágenes en la biblioteca",
  "No items added in your cart. Please add product to your cart list.":
    "No hay artículos en tu carrito. Por favor agrega productos a tu carrito.",
  "No items available": "No hay artículos disponibles",
  "No location found.": "No se encontró ubicación.",
  "No messages yet": "Aún no hay mensajes",
  "No messages yet": "Aún no hay mensajes",
  "No module available": "No hay módulo disponible",
  "No module found": "لم يتم العثور على وحدة",
  "No monthly cart items found": "No monthly cart items found",
  "No password provided.": " لم يتم تقديم كلمة مرور.",
  "No payment info submitted.": "No payment info submitted.",
  "No payment method available.": "No hay método de pago disponible.",
  "No problem if you can't understand the doctor's handwriting, our Pharmacist will assist you in ordering the medicine by viewing the prescription.":
    "No hay problema si no entiendes la letra del doctor, nuestro farmacéutico te ayudará a pedir el medicamento viendo la receta.",
  "No recent address found.": "No se encontraron direcciones recientes.",
  "No restaurant found": "لم يتم العثور على مطعم",
  "No restaurant found": "لم يتم العثور على مطعم",
  "No restaurants found": "No se encontraron restaurantes",
  "No results found": "No results found",
  "No results found": "No results found",
  "No results found for": "No se encontraron resultados para",
  "No reviews found": "No se encontraron reseñas",
  "No reviews yet": "Aún no hay reseñas",
  "No saved addresses found to select.":
    "No se encontraron direcciones guardadas para seleccionar.",
  "No saved addresses found.": "No se encontraron direcciones guardadas.",
  "No saved addresses yet": "Aún no hay direcciones guardadas",
  "No seller info provided.": "No seller info provided.",
  "No stores found": "No stores found",
  "No terms available.": "No hay términos disponibles.",
  "No title given": "لم يتم إعطاء عنوان",
  "No top rated items found": "No se encontraron artículos mejor calificados",
  "No seller info provided.": "No seller info provided.",
  "No stores found": "No stores found",
  "No terms available.": "No hay términos disponibles.",
  "No title given": "لم يتم إعطاء عنوان",
  "No top rated items found": "No se encontraron artículos mejor calificados",
  "No transaction found": "No se encontraron transacciones",
  "No unit type": "Sin tipo de unidad",
  "No vehicles added in your cart. Please add vehicle to your cart list.":
    "No hay vehículos en tu carrito. Por favor agrega un vehículo a tu carrito.",
  "No, Keep this ": " لا ، احتفظ بهذا ",
  "No,Cancel": "No, cancelar",
  "Non Air Conditioned": "Sin aire acondicionado",
  "Non Veg": "No vegetariano",
  "Non-Veg": "غير نباتي",
  "Not Available now": "غير متاح الآن",
  "Non-Veg": "غير نباتي",
  "Not Available now": "غير متاح الآن",
  "Not Changeable": "No modificable",
  "Not Now": "Ahora no",
  "Not changeable": "غير قابل للتغيير",
  "Not changeable": "غير قابل للتغيير",
  "Not found": "No encontrado",
  Note: "ملحوظة",
  "Note !": "¡Nota!",
  "Note Please upload a doctor verified prescription and you can upload jpg, Png":
    "Nota: Por favor sube una receta verificada por un doctor, puedes subir jpg, Png",
  "Note must not be grater than 191 characters":
    "La nota no debe exceder los 191 caracteres",
  "Note:": "Nota:",
  "Note:-": "Nota:-",
  "Nothing found": "لم يتم العثور على شيء",
  Notification: "إشعارات",
  "Notify me when it is back": "أعلمني عندما يعود",
  Now: "حاليا",
  "Now you can track your products easily whenever you want.":
    "Ahora puedes rastrear tus productos fácilmente cuando quieras.",
  "Nutrition Details": "Detalles nutricionales",
  OFF: "إيقاف",
  OK: "Aceptar",
  OR: "O",
  "OR SHARE": "O COMPARTIR",
  OTP: "OTP",
  "OTP Verification": "Verificación OTP",
  Off: "عن",
  Offer: "عرض",
  Offers: "Ofertas",
  Office: "مكتب",
  "Offline Payment now, with just a click of a button, you can make secure transactions. It's simple, convenient, and reliable.":
    "Pago offline ahora, con un solo clic puedes realizar transacciones seguras. Es simple, conveniente y confiable.",
  "Offline Payment! Now, with just a click of a button, you can make secure transactions. It's simple, convenient, and reliable.":
    "¡Pago offline! Ahora, con un solo clic puedes realizar transacciones seguras. Es simple, conveniente y confiable.",
  Ok: "Ok",
  Okay: "تمام",
  On: "Activado",
  "On Demand Service": " خدمة عند الطلب ",
  "On-going": "En curso",
  'One of the vehicles in your list doesn\'t have a ${\n              cartLists?.user_data?.trip_type === "distance"\n                ? "distance wise"\n                : cartLists?.user_data?.trip_type === "hourly"\n                  ? "hourly"\n                  : "day wise"\n            } trip. If you proceed with a ${\n              cartLists?.user_data?.trip_type === "distance"\n                ? "distance wise"\n                : cartLists?.user_data?.trip_type === "hourly"\n                  ? "hourly"\n                  : "day wise"\n            } trip, this vehicle will be removed.':
    'One of the vehicles in your list doesn\'t have a ${\n              cartLists?.user_data?.trip_type === "distance"\n                ? "distance wise"\n                : cartLists?.user_data?.trip_type === "hourly"\n                  ? "hourly"\n                  : "day wise"\n            } trip. If you proceed with a ${\n              cartLists?.user_data?.trip_type === "distance"\n                ? "distance wise"\n                : cartLists?.user_data?.trip_type === "hourly"\n                  ? "hourly"\n                  : "day wise"\n            } trip, this vehicle will be removed.',
  "One or more item is not available for the chosen preferable schedule time.":
    "عنصر واحد أو أكثر غير متاح لوقت الجدول المفضل المختار.",
  "Online · replies instantly": "En línea · responde al instante",
  Only: "Solo",
  "Only For": "فقط ل",
  "Only for": "فقط ل",
  "Only for  Restaurant": "فقط للمطعم",
  "Only for First Order": "فقط للطلب الأول",
  "Only for Pro members": "Only for Pro members",
  "Only for some specific zones": "فقط لبعض المناطق المحددة",
  "Only image and PDF files are allowed":
    "Solo se permiten imágenes y archivos PDF",
  "Oops!": "¡Ups!",
  "Open Confirm Dialog": "افتح حوار التأكيد",
  "Open Delete Dialog": " افتح مربع حوار الحذف ",
  "Open Confirm Dialog": "افتح حوار التأكيد",
  "Open Delete Dialog": " افتح مربع حوار الحذف ",
  "Open Now": "Abierto ahora",
  "Open Your Own Virtual Restaurant On":
    "Abre tu propio restaurante virtual en",
  "Open Your Own Virtual Restaurant On ": "افتح مطعمك الافتراضي على ",
  "Open your own  virtual restaurant on": "افتح مطعمك الافتراضي على",
  Optional: "Opcional",
  "Optional extras you can add to your order":
    "Extras opcionales que puedes agregar a tu pedido",
  Or: "أو",
  "Or Add Custom Note": "O agrega una nota personalizada",
  "Or Custom Massage": "O mensaje personalizado",
  Order: "ترتيب",
  "Order Amount": " كمية الطلب ",
  "Order Confirmed": "تم تاكيد الطلب",
  "Order Details": "Detalles del pedido",
  "Order ID": " رقم التعريف الخاص بالطلب ",
  "Order ID": " رقم التعريف الخاص بالطلب ",
  "Order ID copied!": "¡ID de pedido copiado!",
  "Order ID is missing": "Falta el ID del pedido",
  "Order ID:": "ID de pedido:",
  "Order Id": "ID de pedido",
  "Order Information :": "Información del pedido:",
  "Order Now": "اطلب الان",
  "Order Now": "اطلب الان",
  "Order OTP": "OTP del pedido",
  "Order Payment Details submitted successfully":
    "Detalles del pago del pedido enviados con éxito",
  "Order Place Failed": "فشل مكان الطلب",
  "Order Placed": "Pedido realizado",
  "Order Placed Successfully": "Pedido realizado con éxito",
  "Order Status": " حالة الطلب ",
  "Order Summary": "Resumen del pedido",
  "Order Summery": "ملخص الطلب",
  "Order Status": " حالة الطلب ",
  "Order Summary": "Resumen del pedido",
  "Order Summery": "ملخص الطلب",
  "Order Time": "Hora del pedido",
  "Order Transaction": "Transacción del pedido",
  "Order date:": "Fecha del pedido:",
  "Order didn't place successfully.": "لم يتم تقديم الطلب بنجاح.",
  "Order didn't place successfully.": "لم يتم تقديم الطلب بنجاح.",
  "Order is successful placed": "Pedido realizado con éxito",
  "Order items is unavailable": "Los artículos del pedido no están disponibles",
  "Order min": "Pide al menos",
  "Order now to enjoy exclusive offer with your":
    "Pide ahora para disfrutar de la oferta exclusiva con tu",
  "Order placed": "تم الطلب",
  "Order placed ": "تم الطلب",
  "Order placed at": "تم تقديم الطلب في",
  "Order placed successfully !": "¡Pedido realizado con éxito!",
  "Order placement failed": "Error al realizar el pedido",
  Orders: " الطلب",
  "Orders Placed": "Pedidos realizados",
  Organic: "عضوي",
  Other: "Otro",
  Others: "آحرون",
  "Out of Stock": "إنتهى من المخزن",
  Over: "Sobre",
  Overview: " ملخص ",
  "Owner Information": "معلومات المالك",
  "Package Benefits": "Beneficios del paquete",
  "Page not found": " الصفحة غير موجودة ",
  Paid: "مدفوع",
  "Paid By": "Pagado por",
  "Paid By Wallet": "Pagado con billetera",
  "Paid by wallet": "Pagado con billetera",
  Parcel: "Paquete",
  "Parcel Category": "فئة الطرد",
  "Parcel Delivery Charge + Return Fare":
    "Cargo de entrega del paquete + tarifa de devolución",
  "Parcel Delivery Information": "معلومات توصيل الطرود",
  "Parcel Info": "معلومات الطرد",
  "Parcel Order": "Pedido de paquete",
  "Parcel Order Placed": "Pedido de paquete realizado",
  "Parcel Order Scheduled": "Pedido de paquete programado",
  "Parcel Order Summery": "Resumen del pedido de paquete",
  "Parcel Returned OTP": "OTP de devolución de paquete",
  "Parcel Type": "Tipo de paquete",
  Passport: "جواز سفر",
  Password: " كلمة المرور ",
  "Password can not be updated while you are logged in by using social logins.":
    "لا يمكن تحديث كلمة المرور أثناء تسجيل الدخول باستخدام عمليات تسجيل الدخول الاجتماعية.",
  "Password is required": "كلمة المرور مطلوبة",
  "Password is too short - should be 6 chars minimum.":
    "كلمة المرور قصيرة جدًا - يجب ألا تقل عن 6 أحرف.",
  "Password is too short - should be 8 chars minimum.":
    " كلمة المرور قصيرة جدًا - يجب ألا تقل عن 8 أحرف.",
  "Password must be at least 6 characters":
    "La contraseña debe tener al menos 6 caracteres",
  "Passwords must match": " يجب ان تتطابق كلمات المرور ",
  "Past Order": "طلب سابق",
  "Past orders": "الطلبات السابقة",
  "Pay Now": "Pagar ahora",
  "Pay Offline": "Pagar offline",
  "Pay Via Online": "Pagar en línea",
  "Pay after service": "Pagar después del servicio",
  "Pay on this account": "Pagar en esta cuenta",
  "Pay via Wallet": "Pagar con billetera",
  "Pay your bill using any of the payment method below and input the required information in the form":
    "Paga tu factura usando cualquiera de los métodos de pago abajo e ingresa la información requerida en el formulario",
  Payment: "Pago",
  "Payment Denied": "Pago denegado",
  "Payment Details": " بيانات الدفع ",
  "Payment Details": " بيانات الدفع ",
  "Payment Error !": "¡Error de pago!",
  "Payment Failed": "عملية الدفع فشلت",
  "Payment Failed": "عملية الدفع فشلت",
  "Payment Info": "Información de pago",
  "Payment Method": "طريقة الدفع او السداد ",
  "Payment Method": "طريقة الدفع او السداد ",
  "Payment Methods": "Métodos de pago",
  "Payment Options": "خيارات الدفع",
  "Payment Status": " حالة السداد ",
  "Payment details": " بيانات الدفع ",
  "Payment Options": "خيارات الدفع",
  "Payment Status": " حالة السداد ",
  "Payment details": " بيانات الدفع ",
  "Payment failed": "Pago fallido",
  "Payment failed !!": "¡¡Pago fallido!!",
  "Payment info updated.": "Información de pago actualizada.",
  "Payment method": " طريقة الدفع او السداد ",
  Pending: " قيد الانتظار ",
  Per: "Por",
  "Per Day": "Por día",
  "Per day": "Por día",
  "Personal Details": "Datos personales",
  "Personal Info": "معلومات شخصية",
  Pharmacy: "Farmacia",
  Phone: " هاتف ",
  "Phone Number is required": "Se requiere el número de teléfono",
  "Phone is required": " الهاتف مطلوب ",
  "Phone number is not valid": " رقم الهاتف غير صالح ",
  "Phone number is required": " رقم الهاتف مطلوب ",
  "Phone number must be exactly 10 digits":
    "El número de teléfono debe tener exactamente 10 dígitos",
  "Phone number required": "Se requiere número de teléfono",
  "Pick Form Map": "اختر خريطة النموذج",
  "Pick Form Map": "اختر خريطة النموذج",
  "Pick Location": "Elegir ubicación",
  "Pick Locations": "اختر المواقع",
  "Pick Locations": "اختر المواقع",
  "Pick from Map": "Elegir del mapa",
  "Pick from map": "اختر من الخريطة",
  "Pick the parcel type that best fits your item":
    "Elige el tipo de paquete que mejor se ajuste a tu artículo",
  "Pick up": "Recoger",
  "Pick your location on the map below":
    "Elige tu ubicación en el mapa de abajo",
  "Pickup Address": "عنوان الاستلام",
  "Pickup Location": "Ubicación de recogida",
  "Pickup Now": "Recoger ahora",
  "Pickup Time": "Hora de recogida",
  "Place Order": "مكان الامر ",
  "Plan Details": "Detalles del plan",
  "Please Choose a delivery address.": "الرجاء اختيار عنوان التسليم.",
  "Place Order": "مكان الامر ",
  "Plan Details": "Detalles del plan",
  "Please Choose a delivery address.": "الرجاء اختيار عنوان التسليم.",
  "Please Try Again": "Por favor intenta de nuevo",
  "Please Wait for Provider Confirmation":
    "Por favor espera la confirmación del proveedor",
  "Please add you delivery location so that we can review if the restaurant is available to deliver in your area or not  ":
    "Por favor agrega tu ubicación de entrega para que podamos verificar si el restaurante puede entregar en tu área",
  "Please add your  delivery location so we can check whether the store delivers to your area.  ":
    "Por favor agrega tu ubicación de entrega para que podamos verificar si la tienda entrega en tu área.",
  "Please add your delivery address":
    "Por favor agrega tu dirección de entrega",
  "Please allow browser location permission":
    "Por favor permite el permiso de ubicación del navegador",
  "Please cancel the order": "يرجى إلغاء النظام",
  "Please choose an option from below to continue with this order":
    "Por favor elige una opción de abajo para continuar con este pedido",
  "Please complete your ongoing orders and ":
    "Por favor completa tus pedidos en curso y ",
  "Please confirm only if the parcel has arrived and everything is in order":
    "Por favor confirma solo si el paquete ha llegado y todo está en orden",
  "Please enter a valid email address": "Por favor ingresa un correo válido",
  "Please enter the registered": "Por favor ingresa el registrado",
  "Please enter the registered email where you want to sent your password recovery OTP.":
    "Please enter the registered email where you want to sent your password recovery OTP.",
  "Please enter the registered phone number where you want to sent your password recovery OTP.":
    "Please enter the registered phone number where you want to sent your password recovery OTP.",
  "Please enter your contact info":
    "Por favor ingresa tu información de contacto",
  "Please enter your register Mobile Number":
    "الرجاء إدخال رقم الهاتف المتحرك المسجل الخاص بك",
  "Please enter your registered mobile number":
    "الرجاء إدخال رقم هاتفك المحمول المسجل",
  "Please exchange more than": "الرجاء صرف أكثر من",
  "Please give a phone number": "يرجى إعطاء رقم هاتف",
  "Please give an amount": "الرجاء إعطاء مبلغ",
  "Please insert a valid email.": "Por favor ingresa un correo válido.",
  "Please keep this Order ID handy for track your order in future We’ve also emailed the details to you":
    "Por favor guarda este ID de pedido para rastrear tu pedido en el futuro. También te enviamos los detalles por correo.",
  "Please login": "Por favor inicia sesión",
  "Please login to continue": "Por favor inicia sesión para continuar",
  "Please login to subscribe": "Please login to subscribe",
  "Please login to use this feature": "Please login to use this feature",
  "Please mark the exact location of your business so customers can easily find you.":
    "Por favor marca la ubicación exacta de tu negocio para que los clientes puedan encontrarte fácilmente.",
  "Please pick sender and receiver location on the map":
    "Please pick sender and receiver location on the map",
  "Please pick the receiver location on the map":
    "Please pick the receiver location on the map",
  "Please pick the sender location on the map":
    "Please pick the sender location on the map",
  "Please place the marker inside the available zones.":
    "Por favor coloca el marcador dentro de las zonas disponibles.",
  "Please provide a phone number":
    "Por favor proporciona un número de teléfono",
  "Please provide an email address":
    "Por favor proporciona un correo electrónico",
  "Please search a location first.": "الرجاء البحث عن موقع أولا.",
  "Please search some keywords.": "الرجاء البحث عن بعض الكلمات الرئيسية.",
  "Please select a delivery destination!":
    "¡Por favor selecciona un destino de entrega!",
  "Please select a payment method": "Por favor selecciona un método de pago",
  "Please select a paymrnt method": "Por favor selecciona un método de pago",
  "Please select a pick up zone": "Por favor selecciona una zona de recogida",
  "Please select a reason": "الرجاء تحديد سبب",
  "Please select a reason": "الرجاء تحديد سبب",
  "Please select a zone": "Por favor selecciona una zona",
  "Please select an option.": "Por favor selecciona una opción.",
  "Please select another delivery location so we can check whether the store delivers to your area.":
    "Por favor selecciona otra ubicación de entrega para que podamos verificar si la tienda entrega en tu área.",
  "Please select cancellation reason":
    "Por favor selecciona el motivo de cancelación",
  "Please select parcel category to proceed":
    "Por favor selecciona la categoría del paquete para continuar",
  "Please select your location to proceed":
    "Por favor selecciona tu ubicación para continuar",
  "Please try again after some time or Contact with our support team.":
    "Por favor intenta de nuevo más tarde o contacta a nuestro equipo de soporte.",
  "Please upload your prescription image": "يرجى تحميل صورة الوصفة الخاصة بك",
  "Please wait sometimes": "من فضلك انتظر احيانا",
  "Point to wallet transfer successfully.": "أشر إلى تحويل المحفظة بنجاح.",
  Popular: "شائع",
  "Popular Food": "الغذاء الشعبي",
  "Popular Food Nearby": " طعام شعبي قريب ",
  "Popular Foods Nearby": "الأطعمة الشعبية المجاورة",
  "Popular Items Nearby": "العناصر الشعبية المجاورة",
  "Popular Restaurants": "مطعم شعبي",
  "Popular Restaurants ": "Restaurantes populares ",
  "Popular Resturant Nearby": "مطاعم شهيرة بالجوار",
  "Popular Store": "متجر شعبي",
  "Popular Stores": "المتاجر الشعبية",
  "Popular Tags": "الكلمات الشعبية",
  "Popular Resturant Nearby": "مطاعم شهيرة بالجوار",
  "Popular Store": "متجر شعبي",
  "Popular Stores": "المتاجر الشعبية",
  "Popular Tags": "الكلمات الشعبية",
  "Popular in this Restaurant!": "¡Popular en este restaurante!",
  "Popular in this store!": "شعبية في هذا المتجر!",
  Pos: "POS",
  "Positive Review": "Reseña positiva",
  "Positive Reviews": "Reseñas positivas",
  "Preferable Time": "الوقت المفضل",
  "Preparing Food": "تحضير الطعام",
  Prescription: "روشتة",
  "Prescription Order": "أمر وصفة طبية",
  "Prescription Required": "Receta requerida",
  Previous: "Anterior",
  "Previous coupon": "Cupón anterior",
  "Previously added restaurant foods have been removed from cart. Now, try again.":
    "تمت إزالة أطعمة المطعم المضافة سابقًا من سلة التسوق. الآن ، حاول مرة أخرى",
  Price: "سعر",
  "Price Range": "نطاق السعر",
  "Price Range ($)": "Rango de precio ($)",
  "Price may vary based on time or distance and total trip cost  calculated accordingly.":
    "El precio puede variar según el tiempo o la distancia y el costo total del viaje se calcula en consecuencia.",
  "Prices and availability may change based on current store updates":
    "Los precios y la disponibilidad pueden cambiar según las actualizaciones actuales de la tienda",
  "Privacy Policy": "سياسة الخصوصية",
  Pro: "Pro",
  "Pro Discount": "Pro Discount",
  "Pro Plan": "Plan Pro",
  "Pro coupon benefit available": "Pro coupon benefit available",
  "Pro coupon benefit unlocked": "Beneficio de cupón Pro desbloqueado",
  "Pro discount applied": "Pro discount applied",
  "Pro member": "Pro member",
  "Pro member discount": "Descuento de miembro Pro",
  "Pro member discount unlocked": "Descuento de miembro Pro desbloqueado",
  "Pro member discount unlocked on orders above {{amount}}":
    "Descuento de miembro Pro desbloqueado en pedidos superiores a {{amount}}",
  Proceed: "Continuar",
  "Proceed To Checkout": "Continuar al pago",
  "Proceed to Checkout": "الشروع في الخروج",
  "Product Details": "تفاصيل المنتج",
  Products: "Productos",
  "Products Left": "Productos restantes",
  Profile: "الملف الشخصي",
  "Profile Image": "Imagen de perfil",
  "Profile Settings": "Configuración del perfil",
  "Profile Settings": "Configuración del perfil",
  "Profile successfully updated": "Perfil actualizado con éxito",
  "Promo Coupon": "Cupón promocional",
  "Provider Details": " تفاصيل المزود ",
  "Provider Details With Cards and Tabs":
    " تفاصيل المزود مع البطاقات وعلامات التبويب",
  Providers: " الموفرون",
  Purchase: "شراء",
  Qty: "الكمية",
  "Quality Medicines & Health care at your Doorstep.":
    "جودة الأدوية والرعاية الصحية على عتبة داركم.",
  Quantity: "كمية",
  "Quick & Emergency Delivery": "Entrega rápida y de emergencia",
  "Quick Action": "Quick Action",
  "Quick Delivery": "Entrega rápida",
  "Quick Links": "روابط سريعة",
  "Quick View": "نظرة سريعة",
  "Quickly find fresh items and get them delivered to your door.":
    "Encuentra rápidamente productos frescos y haz que te los entreguen en tu puerta.",
  "Rate the deliveryman": "قيم عامل التوصيل",
  "Rate the food": "قيم الطعام",
  "Rate the item": "Califica el artículo",
  Rating: "تقييم",
  Ratings: "التقييمات",
  "Ratings & Reviews": "Ratings & Reviews",
  "Re-enter your password": "Vuelve a ingresar tu contraseña",
  "Re-order": "Reordenar",
  "Ready to Ride": "Listo para Viajar",
  Receiver: "المتلقي",
  "Receiver Details": "Detalles del receptor",
  "Receiver Info": "Información del receptor",
  "Receiver Information": "معلومات استقبال",
  "Receiver Information": "معلومات استقبال",
  "Receiver Location": "Ubicación del receptor",
  "Receiver Name": "اسم المتلقي",
  "Receiver Phone": "هاتف المستقبل",
  "Receiver Name": "اسم المتلقي",
  "Receiver Phone": "هاتف المستقبل",
  "Receiver name required": "Se requiere el nombre del receptor",
  "Receiver phone required": "Se requiere el teléfono del receptor",
  "Recent Searches": "Búsquedas recientes",
  "Recent address": "Dirección reciente",
  "Recent chats": "Chats recientes",
  Recommend: "Recomendar",
  "Recommend a popular dish": "Recomienda un plato popular",
  "Recommend a popular product": "Recomienda un producto popular",
  "Recommend common health essentials": "Recomienda artículos básicos de salud",
  "Recommend popular grocery items":
    "Recomienda artículos populares de comestibles",
  "Recommend something popular": "Recomienda algo popular",
  RecommendProduct: "يوصي المنتج",
  Recommended: "مُستَحسَن",
  "Recommended For You": "Recomendado para ti",
  "Recommended Pharmacies": "Farmacias recomendadas",
  "Recommended Store": "Tienda recomendada",
  "Recommended for you": "Recomendado para ti",
  Reels: "Reels",
  "Refer Code": "Código de referido",
  "Refer Code (Optional)": "إحالة الكود (اختياري)",
  "Refer your code to your friends": "قم بإحالة الكود الخاص بك إلى أصدقائك",
  "Refer your code to your friends and get":
    "Comparte tu código con tus amigos y obtén",
  "Referral Code": "Código de referido",
  "Referral Discount": "Descuento por referido",
  "Referral code Copied": "تم نسخ رمز الإحالة",
  "Referral code Copied": "تم نسخ رمز الإحالة",
  "Referrer Transactions": "Transacciones de referidos",
  "Refill Request": "طلب إعادة التعبئة",
  "Refill Request": "طلب إعادة التعبئة",
  "Refill Your Medicine": "Reabastece tu medicamento",
  "Refresh the page if needed": "Actualiza la página si es necesario",
  Refund: "Reembolso",
  "Refund Policy": "سياسة الاسترجاع",
  "Refund Request": "طلب ارجاع",
  Register: "يسجل",
  "Register as delivery man and earn money": "سجل كرجل توصيل واكسب المال",
  "Register as seller and open shop in": "سجل كبائع وافتح متجرًا في",
  "Register as seller and open shop in 6amMart to start your business":
    "سجل كبائع وافتح متجرًا في 6ammart لبدء عملك",
  "Remaining Balance": "Saldo restante",
  "Remaining Bill": "Factura restante",
  "Remaining Wallet Balance": "Saldo restante de la billetera",
  "Remember me": " تذكرنى؟",
  Remove: "Eliminar",
  "Remove & Continue": "Eliminar y continuar",
  "Remove Monthly Order": "Remove Monthly Order",
  "Remove from wishlist": "إزالة من قائمة الرغبات",
  "Remove it form my cart": "إزالته من عربة التسوق",
  "Removed from  favorite successfully.": "تمت الإزالة من المفضلة بنجاح.",
  "Remove Monthly Order": "Remove Monthly Order",
  "Remove from wishlist": "إزالة من قائمة الرغبات",
  "Remove it form my cart": "إزالته من عربة التسوق",
  "Removed from  favorite successfully.": "تمت الإزالة من المفضلة بنجاح.",
  "Removed from cart.": "Eliminado del carrito.",
  "Removed from wishlist": "Removed from wishlist",
  "Removing...": "Removing...",
  Renew: "Renovar",
  "Renew Subscription": "Renovar suscripción",
  "Renew Your Subscription": "Renueva tu suscripción",
  Rent: "Alquilar",
  "Rent This Car": "Alquilar este auto",
  "Rent Type": "Tipo de alquiler",
  "Rent best car for best experience":
    "Alquila el mejor auto para la mejor experiencia",
  "Rent the Perfect Car for Every Journey":
    "Alquila el coche perfecto para cada viaje",
  "Reordering from {{name}} will clear your current restaurant cart. Do you want to continue?":
    "Reordenar desde {{name}} eliminará tu carrito actual del restaurante. ¿Deseas continuar?",
  "Reordering from {{name}} will clear your current store cart. Do you want to continue?":
    "Reordenar desde {{name}} eliminará tu carrito actual de la tienda. ¿Deseas continuar?",
  "Reply By": "Respuesta de",
  "Request OTP": "Solicitar OTP",
  Required: "Requerido",
  "Resend it": "Reenviar",
  Reset: " إعادة ضبط ",
  Restaurant: "مطعم",
  "Restaurant Address": "عنوان المطعم",
  "Restaurant Information's": "معلومات المطعم",
  "Restaurant Informations": "معلومات المطعم",
  "Restaurant Name": "اسم المطعم",
  "Restaurant Registration": "تسجيل مطعم",
  "Restaurant address required": "Se requiere la dirección del restaurante",
  "Restaurant is closed. Try again later.":
    "El restaurante está cerrado. Inténtalo de nuevo más tarde.",
  "Restaurant name required": "Se requiere el nombre del restaurante",
  Restaurants: " مطاعم ",
  "Resturant Informations": " معلومات المطعم ",
  Resturants: "مطاعم",
  "Result For": "Resultado para",
  "Results Found": "Results Found",
  "Results Found": "Results Found",
  "Return Fare": "Tarifa de devolución",
  "Return fee": "Tarifa de devolución",
  Rev: "Rev",
  Review: "Reseña",
  "Review Your Order": " راجع طلباتك ",
  "Review your order": " راجع طلباتك ",
  Reviews: " المراجعات ",
  Ride: "Ride",
  "Ride Anywhere,": "Viaja a Cualquier Lugar,",
  Road: "Calle",
  "Rows Per Page": " عدد الصفوف في الصفحة ",
  "Run store by purchasing subsciption  packages. You will have access the features of in store panel , app and interaction with user according to the subscription packages.":
    "Gestiona la tienda comprando paquetes de suscripción. Tendrás acceso a las funciones del panel de la tienda, la app e interacción con los usuarios según el paquete de suscripción.",
  Running: "En curso",
  "Running Campaigns": "الحملات الجارية",
  "Rx Accepted": "Receta aceptada",
  "SERVICE MANAGEMENT": " إدارة الخدمات ",
  "SOME COMPONENTS": " بعض المكونات",
  Salary: "مرتب",
  "Same as general information": " نفس المعلومات العامة ",
  "Same vehicle": "Mismo vehículo",
  Save: "Guardar",
  "Save Address": "Guardar dirección",
  "Save It For Later": "Guardar para más tarde",
  "Save for your next order": "Guardar para tu próximo pedido",
  "Save more on every order": "Save more on every order",
  "Save on delivery fees as a Pro member":
    "Ahorra en gastos de envío como miembro Pro",
  "Save on delivery fees as a Pro member on deliveries above {{amount}}":
    "Ahorra en gastos de envío como miembro Pro en entregas superiores a {{amount}}",
  "Save this Address": "Guardar esta dirección",
  "Save your favourite items and stores here":
    "Guarda tus artículos y tiendas favoritas aquí",
  "Saved Addresses": "العناوين المحفوظة",
  "Saves Addresses": "يحفظ العناوين",
  "Scan to Download": "Escanea para descargar",
  Schedule: "برنامج",
  "Schedule Booking Request Successful!":
    "¡Solicitud de reserva programada con éxito!",
  "Schedule Delivery": "Programar entrega",
  "Schedule at": "Programar para",
  "Scroll Up": "Subir",
  "Scroll Up": "Subir",
  "Scroll categories left": "Desplazar categorías a la izquierda",
  "Scroll categories right": "Desplazar categorías a la derecha",
  "Scroll left": "Desplazar a la izquierda",
  "Scroll right": "Desplazar a la derecha",
  Search: "يبحث",
  "Search Delicious Food": "Busca Comida Deliciosa",
  "Search Delicious Offers": "Busca ofertas de comida deliciosa",
  "Search Deliveryman": "Buscar repartidor",
  "Search Express Delivery Food": "Busca comida con entrega exprés",
  "Search Express Delivery Grocery": "Busca supermercado con entrega exprés",
  "Search Express Delivery Products": "Busca productos con entrega exprés",
  "Search Express Delivery Food": "Busca comida con entrega exprés",
  "Search Express Delivery Grocery": "Busca supermercado con entrega exprés",
  "Search Express Delivery Products": "Busca productos con entrega exprés",
  "Search For": "Buscar",
  "Search Fresh Essentials Now": "Busca Productos Frescos Ahora",
  "Search Grocery Items": "Busca productos de supermercado",
  "Search Grocery Offers": "Busca ofertas de supermercado",
  "Search Here...": "Buscar aquí...",
  "Search Medicine Offers": "Busca ofertas de medicamentos",
  "Search Medicines": "Busca medicamentos",
  "Search Nearby Grocery Stores": "Busca tiendas de supermercado cercanas",
  "Search Nearby Pharmacies": "Busca farmacias cercanas",
  "Search Nearby Restaurants": "Busca restaurantes cercanos",
  "Search Nearby Shops": "Busca tiendas cercanas",
  "Search Fresh Essentials Now": "Busca Productos Frescos Ahora",
  "Search Grocery Items": "Busca productos de supermercado",
  "Search Grocery Offers": "Busca ofertas de supermercado",
  "Search Here...": "Buscar aquí...",
  "Search Medicine Offers": "Busca ofertas de medicamentos",
  "Search Medicines": "Busca medicamentos",
  "Search Nearby Grocery Stores": "Busca tiendas de supermercado cercanas",
  "Search Nearby Pharmacies": "Busca farmacias cercanas",
  "Search Nearby Restaurants": "Busca restaurantes cercanos",
  "Search Nearby Shops": "Busca tiendas cercanas",
  "Search Order": "Buscar pedido",
  "Search Product Offers": "Busca ofertas de productos",
  "Search Products": "Busca productos",
  "Search Saved Grocery": "Busca en supermercado guardado",
  "Search Saved Medicine": "Busca en medicamentos guardados",
  "Search Saved Products": "Busca en productos guardados",
  "Search Saved Restaurants": "Busca en restaurantes guardados",
  "Search Top Rated Grocery Stores":
    "Busca las mejores tiendas de supermercado",
  "Search Top Rated Pharmacies": "Busca las farmacias mejor valoradas",
  "Search Top Rated Restaurants": "Busca los restaurantes mejor valorados",
  "Search Top Rated Shops": "Busca las tiendas mejor valoradas",
  "Search Trip": "Buscar viaje",
  "Search Verified Seller Shops": "Busca tiendas de vendedores verificados",
  "Search a category": "ابحث عن فئة",
  "Search a restaurant": "ابحث عن مطعم",
  "Search by Health Condition": "Busca por condición de salud",
  "Search foods in restaurant": "البحث عن الأطعمة في المطعم",
  "Search foods in restaurant ": "ابحث عن الأطعمة في المطعم",
  "Search for": "Buscar",
  "Search for authentic medicines and healthcare essentials delivered to your doorstep.":
    "Busca medicamentos auténticos y productos esenciales de salud entregados en tu puerta.",
  "Search for items or store...": "البحث عن العناصر أو المتجر ...",
  "Search for items...": "البحث عن العناصر ...",
  "Search for stores...": "Buscar tiendas...",
  "Search from here": "Buscar desde aquí",
  "Search in {{name}}": "Busca en {{name}}",
  "Search in {{name}}": "Busca en {{name}}",
  "Search location": "Buscar ubicación",
  "Search location here...": "ابحث عن الموقع هنا ...",
  "Search medicines by name": "Busca medicamentos por nombre",
  "Search result for": "نتيجة البحث عن",
  "Search suggestions are loading...": "يتم تحميل اقتراحات البحث ...",
  "Search location here...": "ابحث عن الموقع هنا ...",
  "Search medicines by name": "Busca medicamentos por nombre",
  "Search result for": "نتيجة البحث عن",
  "Search suggestions are loading...": "يتم تحميل اقتراحات البحث ...",
  "Search vehicles...": "Buscar vehículos...",
  "Search your desired foods ": "ابحث عن الأطعمة التي تريدها ",
  "Search...": "يبحث",
  Seats: "Asientos",
  Sec: "Seg",
  "See Chat History": "Ver historial de chat",
  "See Less": "Ver menos",
  "See More": "Ver más",
  "See all": "Ver todo",
  "See less": "Ver menos",
  "See more": "شاهد المزيد",
  "See our most popular restaurant and foods":
    "Ve nuestros restaurantes y comidas más populares",
  Select: "Seleccionar",
  "Select  Sub Categories": " حدد الفئات الفرعية ",
  "Select  Zones": " حدد المناطق ",
  'Select "Allow" when prompted for microphone access':
    'Select "Allow" when prompted for microphone access',
  "Select And Start meaningful conversations!": "حدد وابدأ محادثات هادفة!",
  "Select Business Module": "Selecciona el módulo del negocio",
  "Select Business Zone": "Selecciona la zona del negocio",
  "Select Categories": " حدد الفئات ",
  "Select Date Range": " حدد النطاق الزمني ",
  "Select Categories": " حدد الفئات ",
  "Select Date Range": " حدد النطاق الزمني ",
  "Select Deliveryman Type": "Selecciona el tipo de repartidor",
  "Select Duration": "Selecciona la duración",
  "Select Identity Type": " حدد نوع الهوية ",
  "Select Identity Type": " حدد نوع الهوية ",
  "Select Payment Method": "Selecciona el método de pago",
  "Select Trip Type": "Selecciona el tipo de viaje",
  "Select Variations": "حدد الاختلافات",
  "Select Variations": "حدد الاختلافات",
  "Select Vehicle Type": "Selecciona el tipo de vehículo",
  "Select Your Time Slot": "Selecciona tu franja horaria",
  "Select Zone": "Selecciona la zona",
  "Select a Payment Method to Proceed":
    "Selecciona un método de pago para continuar",
  "Select a file ": "Selecciona un archivo ",
  "Select a file or": "Selecciona un archivo o",
  "Select a file or Drag & Drop here":
    "Selecciona un archivo o arrastra y suelta aquí",
  "Select a payment method": "Selecciona un método de pago",
  "Select a payment method to proceed.": "Select a payment method to proceed.",
  "Select a saved address or choose a new location":
    "Selecciona una dirección guardada o elige una nueva ubicación",
  "Select a type of module for your orders": "حدد نوع الوحدة لأوامرك",
  "Select an option": "حدد اختيارا",
  "Select an option from below.": "Select an option from below.",
  "Select any of": "Selecciona cualquiera de",
  "Select duration": "Selecciona duración",
  "Select from saved addresses or pick from map":
    "Selecciona desde direcciones guardadas o elige del mapa",
  "Select images to upload": "Selecciona imágenes para subir",
  "Select location first to start exploring shops & restaurants near you":
    "حدد الموقع أولاً لبدء استكشاف المتاجر والمطاعم القريبة منك",
  "Select option from below": "Selecciona una opción de abajo",
  "Select the Reason for Support": "Selecciona el motivo del soporte",
  "Select the type of stores for your order": "حدد نوع المتاجر لطلبك",
  "Select the type of stores for your order": "حدد نوع المتاجر لطلبك",
  "Select what you wish to send": "Selecciona qué deseas enviar",
  "Select your instruction": "Selecciona tu instrucción",
  "Select your location": "Selecciona tu ubicación",
  Selected: "Seleccionado",
  "Selected Address": "العنوان المحدد",
  "Selected variation named": "الشكل المحدد المسمى",
  "Selecting a vehicle from another provider will remove all vehicles from the cart and add the new one.":
    "Seleccionar un vehículo de otro proveedor eliminará todos los vehículos del carrito y agregará el nuevo.",
  "Self delivery": "Entrega propia",
  "Self-Care Medicines": "Medicamentos de autocuidado",
  Seller: "Vendedor",
  "Seller App": "تطبيق البائع",
  "Seller Payment Info": "Información de pago del vendedor",
  Send: "يرسل",
  "Send  Fastest Parcel Delivery":
    "Envía con la entrega de paquetes más rápida",
  "Send Massage": "Enviar mensaje",
  "Send To": "Send To",
  "Send us mails": "أرسل لنا رسائل بريد إلكتروني",
  "Send your parcel anywhere in country instantly & Safely":
    "أرسل طردك إلى أي مكان في البلد على الفور وبأمان",
  "Send your wallet money while order":
    "Envía dinero de tu billetera al realizar el pedido",
  Sender: "مرسل",
  "Sender Details": "Detalles del remitente",
  "Sender Info": "Información del remitente",
  "Sender Information": "معلومات المرسل",
  "Sender Name": "اسم المرسل",
  "Sender Phone": "هاتف المرسل",
  "Sender Information": "معلومات المرسل",
  "Sender Name": "اسم المرسل",
  "Sender Phone": "هاتف المرسل",
  "Sender name required": "Se requiere el nombre del remitente",
  "Sender or Receiver location is missing":
    "Falta la ubicación del remitente o receptor",
  "Sender phone required": "Se requiere el teléfono del remitente",
  "Service Categories": " فئات الخدمة ",
  "Service Categories": " فئات الخدمة ",
  "Service Fee": "Tarifa de servicio",
  "Service List": " قائمة الخدمات ",
  "Service Man List": " قائمة رجل الخدمة ",
  "Service Zones": " مناطق الخدمة ",
  Services: " خدمات",
  "Set Location": "تحديد المكان",
  "Set Your Business Location on Map":
    "Establece la ubicación de tu negocio en el mapa",
  "Set from map": "تعيين من الخريطة",
  "Set your location": "Establece tu ubicación",
  Settings: "Configuración",
  Share: "Compartir",
  "Share us your valuable feedbacks": "Comparte tu valiosa opinión",
  "Share your opinion": "مشاركة رأيك",
  "Sharing your accurate location enhances precision in search results and delivery estimates, ensures effortless order delivery.":
    "Compartir tu ubicación precisa mejora los resultados de búsqueda y las estimaciones de entrega, asegurando una entrega sin esfuerzo.",
  "Sharing your location improves search accuracy and delivery estimates for smoother order delivery.":
    "Compartir tu ubicación mejora la precisión de búsqueda y las estimaciones de entrega para una entrega más fluida.",
  "Shift Plan": "Cambiar plan",
  "Shipping Address": " عنوان الشحن ",
  "Shipping Policy": "سياسة الشحن",
  "Shipping address": " عنوان الشحن ",
  "Shipping policy": "سياسة الشحن",
  Shop: "Tienda",
  "Shop & Save Up to {{discount}} Off?": "¡Compra y ahorra hasta {{discount}}!",
  "Shop By Conditions": "Comprar por condición",
  "Shop Now": "تسوق الآن",
  "Shop Your Favorites Now": "Compra Tus Favoritos Ahora",
  "Shop by Categories": "Comprar por categorías",
  "Shopping Cart": "عربة التسوق",
  Shops: "Tiendas",
  "Short By": "Ordenar por",
  "Show Reply": "Mostrar respuesta",
  "Show me available pharmacy offers":
    "Muéstrame las ofertas de farmacia disponibles",
  "Show me today's best deals": "Muéstrame las mejores ofertas de hoy",
  "Show me today's best food deals":
    "Muéstrame las mejores ofertas de comida hoy",
  "Show me today's grocery deals":
    "Muéstrame las ofertas de comestibles de hoy",
  "Show me today's shop deals": "Muéstrame las ofertas de tiendas de hoy",
  "Sign In": "تسجيل الدخول",
  "Sign Up": "اشتراك",
  "Sign in successfully.": "تسجيل الدخول بنجاح.",
  "Sign in to stay connected.": " تسجيل الدخول للبقاء على اتصال.",
  "Sign in with": "Iniciar sesión con",
  "Signup successfully.": "Registro exitoso.",
  Similar: "Similar",
  "Similar Products": "منتجات مماثلة",
  Size: "Tamaño",
  "Skip for Now": "Omitir por ahora",
  "Slightly Delay Delivery": "Slightly Delay Delivery",
  "Slightly Delay Delivery": "Slightly Delay Delivery",
  "Smart shopping starts here.": "Las compras inteligentes comienzan aquí.",
  "Social Login": "تسجيل الدخول الاجتماعي",
  "Social Register": "السجل الاجتماعي",
  Sold: "Vendido",
  Solution: "حل",
  "Some files are already added": "Algunos archivos ya están agregados",
  "Someone just used  your code !": "¡Alguien acaba de usar tu código!",
  "Something went wrong": "Something went wrong",
  "Something went wrong.": "هناك خطأ ما.",
  "Something went wrong": "Something went wrong",
  "Something went wrong.": "هناك خطأ ما.",
  "Sorry !": "¡Lo sentimos!",
  "Sorry no data found related to your search":
    "عذرا ، لم يتم العثور على بيانات متعلقة ببحثك",
  "Sorry you can’t delete your account !":
    "¡Lo sentimos, no puedes eliminar tu cuenta!",
  "Sorry, Something Went Wrong": "Lo sentimos, algo salió mal",
  "Sort By": "ترتيب حسب",
  "Sort Grid": "فرز الشبكة",
  "Sort by": "صنف حسب",
  "Sort By": "ترتيب حسب",
  "Sort Grid": "فرز الشبكة",
  "Sort by": "صنف حسب",
  "Sort by : A to Z": "Ordenar por: A a Z",
  "Sort by : Default": "Ordenar por: Predeterminado",
  "Sort by : High to Low": "فرز حسب: من الأعلى إلى الأقل",
  "Sort by : High to Low": "فرز حسب: من الأعلى إلى الأقل",
  "Sort by : Low to High": "Ordenar por: De menor a mayor",
  "Sort by : Z to A": "Ordenar por: Z a A",
  "Sort by:": "Ordenar por:",
  "Sort by: A to Z": "Ordenar por: A a Z",
  "Sort by: Default": "Ordenar por: Predeterminado",
  "Sort by: Z to A": "Ordenar por: Z a A",
  "Special Food Offers": "عروض الطعام الخاصة",
  "Special Food Offers": "عروض الطعام الخاصة",
  "Special Offer": "Oferta especial",
  Sponsored: "Patrocinado",
  "Standard Delivery": "Standard Delivery",
  "Start Exploring!": "ابدأ الاستكشاف!",
  "Start Free Trial": "Iniciar prueba gratis",
  "Start From": "Desde",
  "Start a New Order?": "¿Iniciar un nuevo pedido?",
  "Start a New Order?": "¿Iniciar un nuevo pedido?",
  "Start a new message": "Iniciar un mensaje nuevo",
  "Start exploring the best services around you.":
    "Comienza a explorar los mejores servicios a tu alrededor.",
  "Start the conversation": "Inicia la conversación",
  "Starts from": "يبدأ من",
  Status: " حالة ",
  "Stay upto date with restaurants and foods around you":
    "ابق على اطلاع على المطاعم والأطعمة من حولك",
  "Still have questions?": "¿Aún tienes preguntas?",
  Store: "متجر",
  "Store Information": "معلومات المتجر",
  "Store closed.": "Tienda cerrada.",
  "Store for you !": "متجر لك!",
  "Store for you !": "متجر لك!",
  "Store is closed.": "La tienda está cerrada.",
  "Store is closed. Try again later.":
    "La tienda está cerrada. Inténtalo de nuevo más tarde.",
  "Store will pay {{commission}}% commission to {{businessName}} from each order. You will get access to all the features and options in store panel, app and interaction with user.":
    "La tienda pagará {{commission}}% de comisión a {{businessName}} de cada pedido. Tendrás acceso a todas las funciones y opciones del panel de la tienda, app e interacción con el usuario.",
  Stores: "المتاجر",
  "Stores not found!": "لم يتم العثور على المتاجر!",
  "Street Number": "رقم الشارع",
  "Street number": "رقم الشارع",
  "Sub Category Setup": "إعداد الفئة الفرعية ",
  Submit: " يُقدِّم  ",
  "Submit & Next": "Enviar y siguiente",
  "Submit Information": "Enviar información",
  "Submit Refund Request": "إرسال طلب استرداد",
  "Submit Refund Request": "إرسال طلب استرداد",
  "Submit search": "Enviar búsqueda",
  Subscribe: "يشترك",
  "Subscribe Now": "Suscribirse ahora",
  "Subscribe to our weekly newsletter and be a part of our journey to self discovery and love.":
    "اشترك في النشرة الإخبارية الأسبوعية وكن جزءًا من رحلتنا لاكتشاف الذات والحب.",
  "Subscribed Services": " الخدمات المشتركة ",
  "Subscribed Successfully": "Suscripción exitosa",
  "Subscribed successfully": "Subscribed successfully",
  "Subscribed successfully": "Subscribed successfully",
  "Subscription Base": "Base de suscripción",
  "Subscription Cancelled": "Suscripción cancelada",
  "Subscription Expired": "Suscripción caducada",
  "Subscription Failed": "Suscripción fallida",
  "Subscription Plan": "Plan de suscripción",
  "Subscription Successful": "Suscripción exitosa",
  "Subscription cancelled": "Suscripción cancelada",
  "Subscription failed": "Suscripción fallida",
  Subtotal: "المجموع الفرعي",
  Suggested: "Sugerido",
  "Suggested Searches": "Búsquedas sugeridas",
  Suggestions: "اقتراحات",
  Summary: "Resumen",
  "Switch Your payment method ": "Cambia tu método de pago ",
  "Switch to COD": "Cambiar a contra entrega",
  "Switch to Cash On Delivery": "قم بالتبديل إلى الدفع النقدي عند التسليم",
  "Switch to Cash On Delivery": "قم بالتبديل إلى الدفع النقدي عند التسليم",
  "Switch to Cash on Delivery": "Cambiar a pago contra entrega",
  "Switch to cash on delivery": "Cambiar a pago contra entrega",
  TAX: "ضريبة",
  "TIN Certificate": "Certificado TIN",
  "TIN Certificate Image is required":
    "Se requiere la imagen del certificado TIN",
  "TIN Expire Date is required": "Se requiere la fecha de vencimiento del TIN",
  "TIN can only contain numbers and symbols":
    "El TIN solo puede contener números y símbolos",
  "TIN cannot exceed 20 characters":
    "El TIN no puede exceder los 20 caracteres",
  "TIN expire date cannot be in the past":
    "La fecha de vencimiento del TIN no puede ser en el pasado",
  "TIN must be at least 3 characters":
    "El TIN debe tener al menos 3 caracteres",
  Tables: " الجداول ",
  "Take Away": "يبعد",
  "Take a Ride": "Pedir un Viaje",
  "Take away": "Para llevar",
  "Tap a coupon to apply, or enter a code below.":
    "Toca un cupón para aplicarlo, o ingresa un código abajo.",
  "Tap apply to redeem.": "Toca aplicar para canjear.",
  "Tap to copy": "انقر للنسخ",
  "Taxpayer Identification Number (TIN)":
    "Número de identificación tributaria (TIN)",
  "Taxpayer Identification Number(TIN) is required":
    "Se requiere el número de identificación tributaria (TIN)",
  "Tell me what you're looking for......": "Dime qué estás buscando......",
  "Terms & Conditions": "البنود و الظروف",
  "Terms & Conditions": "البنود و الظروف",
  "Terms And Conditions": "Términos y condiciones",
  "Terms and Condition": "Términos y condiciones",
  "Terms and Conditions": "الأحكام والشروط",
  "Terms and conditions": "الأحكام والشروط",
  "Terms and conditions are currently unavailable.":
    "Los términos y condiciones no están disponibles actualmente.",
  "The browser will refresh to get translated content.":
    "سيتم تحديث المتصفح للحصول على المحتوى المترجم.",
  "The driver has reached the destination":
    "El conductor ha llegado al destino",
  "The service": "El servicio",
  "The total amount of the order, exceeds maximum amount of the cash on delivery method":
    "المبلغ الإجمالي للطلب ، يتجاوز الحد الأقصى لمبلغ الدفع نقدًا عند التسليم",
  Theme: "Tema",
  "Theme Mode": "Modo de tema",
  "Theme Settings": "إعدادات الموضوع",
  "Theme mode": "وضع الموضوع",
  "Theme Settings": "إعدادات الموضوع",
  "Theme mode": "وضع الموضوع",
  "They create a account on": "Crean una cuenta en",
  "They create a account on 6amMart using your code and place their first order":
    "Crean una cuenta en 6amMart usando tu código y realizan su primer pedido",
  This: "هذا",
  "This Variation needs to be selected in between minimum":
    "يجب تحديد هذا الاختلاف بين الحد الأدنى",
  "This action cannot be undone.": "Esta acción no se puede deshacer.",
  "This action cannot be undone. Are you sure you want to delete all saved files?":
    "Esta acción no se puede deshacer. ¿Estás seguro que quieres eliminar todos los archivos guardados?",
  "This amount is for service maintenance.":
    "Este monto es para el mantenimiento del servicio.",
  "This charge includes extra vehicle charge":
    "Este cargo incluye el cargo extra del vehículo",
  "This delivery fee includes all the applicable charges on delivery":
    "Esta tarifa de entrega incluye todos los cargos aplicables",
  "This field is required": " هذه الخانة مطلوبه ",
  "This item is no longer available. It may have been removed or replaced by the store.":
    "This item is no longer available. It may have been removed or replaced by the store.",
  "This operation cannot be undone": " هذه العملية لا يمكن التراجع عنها ",
  "This product is Halal": "Este producto es Halal",
  "This store is not available in your location":
    "Esta tienda no está disponible en tu ubicación",
  "This variation is out of stock": "هذا الشكل غير متوفر",
  "This variation is out of stock. Choose another variation to proceed further.":
    "Esta variación está agotada. Elige otra variación para continuar.",
  "This vehicle is not available in this pick time":
    "Este vehículo no está disponible en esta hora de recogida",
  Time: "زمن",
  "To apply this coupon, total item amount should be minimum":
    "لتطبيق هذه القسيمة ، يجب أن يكون إجمالي مبلغ العنصر هو الحد الأدنى",
  "To apply this coupon, total order amount should be minimum":
    "Para aplicar este cupón, el monto total del pedido debe ser mínimo",
  "To get more personalised & smooth experience please log in or sign up":
    "Para una experiencia más personalizada y fluida, por favor inicia sesión o regístrate",
  "To know others information you may": "Para conocer más información puedes",
  "To order with us add anything to your cart.":
    "Para hacer un pedido, agrega lo que quieras a tu carrito.",
  "To save more use available coupons":
    "Para ahorrar más usa los cupones disponibles",
  "To select from saved addresses, you need to sign in.":
    "Para seleccionar desde direcciones guardadas, debes iniciar sesión.",
  Toasters: " محمصات ",
  Today: "اليوم",
  "Todays Deals": "Ofertas de hoy",
  Tomorrow: "الغد",
  "Top Brands": "Mejores marcas",
  "Top Picks": "Mejores opciones",
  "Top Picks Near You": "Mejores opciones cerca de ti",
  "Top Rated": "Mejor calificado",
  "Top Rated Vehicles": "Vehículos mejor calificados",
  "Top Restaurants": "Mejores restaurantes",
  "Top Store": "Mejor tienda",
  "Top offers near me": "Mejores ofertas cerca de mí",
  Total: "المجموع",
  "Total Amount": "المبلغ الإجمالي",
  "Total Balance": "Saldo total",
  "Total Bill": "Factura total",
  "Total Hour": "Total de horas",
  "Total Orders": "Pedidos totales",
  "Total Price": "السعر الكلي",
  "Total Saved": "Total ahorrado",
  "Total Price": "السعر الكلي",
  "Total Saved": "Total ahorrado",
  "Total points": "Puntos totales",
  "Total trip cost": "Costo total del viaje",
  "Track Now": "Rastrear ahora",
  "Track Order": "Rastrear pedido",
  "Track Orders": "Rastrear pedidos",
  "Track Trip": "Rastrear viaje",
  "Track Your Order": "Rastrea tu pedido",
  "Track Your Trip": "Rastrea tu viaje",
  "Track order": "Rastrear pedido",
  "Track your Products": "Rastrea tus productos",
  "Track your order": "Rastrea tu pedido",
  "Transaction History": "تاريخ المعاملات",
  "Transaction History": "تاريخ المعاملات",
  "Transaction Type": "Tipo de transacción",
  "Trending Bites": "Lo más visto y pedido",
  "Trending Bites": "Lo más visto y pedido",
  "Trending Dishes": "Platillos en tendencia",
  "Trending Items": "Artículos en tendencia",
  "Trending Now": "Tendencias Ahora",
  "Trending Now": "Tendencias Ahora",
  "Trending Searches": "Búsquedas en tendencia",
  Trip: "Viaje",
  "Trip Booking Request Successful!": "¡Solicitud de reserva de viaje exitosa!",
  "Trip Cart": "Carrito de viaje",
  "Trip Cost": "Costo del viaje",
  "Trip Details": "Detalles del viaje",
  "Trip Discount": "Descuento del viaje",
  "Trip Duration (day)": "Duración del viaje (día)",
  "Trip Duration (hrs)": "Duración del viaje (hrs)",
  "Trip Id": "ID del viaje",
  "Trip Route": "طريق الرحلة",
  "Trip Route": "طريق الرحلة",
  "Trip Type": "Tipo de viaje",
  "Trip Vehicle List": "Lista de vehículos del viaje",
  "Trip details": "Detalles del viaje",
  "Trust & secure buying experience.":
    "Experiencia de compra confiable y segura.",
  "Trusted by thousands of Users worldwide":
    "Confiado por miles de usuarios en todo el mundo",
  "Try one of these to get started, or write your own.":
    "Prueba una de estas para empezar, o escribe la tuya.",
  "Try to ": "Intenta ",
  Type: "Tipo",
  "Type Here": "Escribe aquí",
  "Type a message...": "Escribe un mensaje...",
  "Type here to write a custom message":
    "Escribe aquí para redactar un mensaje personalizado",
  "Type your email": "Escribe tu correo",
  "Type your number or mail": "Escribe tu número o correo",
  "Type your opinion": "Escribe tu opinión",
  "Type your tin number": "Escribe tu número TIN",
  "Typing…": "Escribiendo…",
  "Unable to renew — plan information unavailable.":
    "Unable to renew — plan information unavailable.",
  "Unavailable item note": "ملاحظة عنصر غير متوفر",
  Unit: "وحدة",
  "Unit Price": "Precio unitario",
  "Unknown status.": "Estado desconocido.",
  "Unlock exclusive benefits, save more on every order, and enjoy free deliveries.":
    "Desbloquea beneficios exclusivos, ahorra más en cada pedido y disfruta de envíos gratis.",
  "Unlock exclusive coupon deals for all orders":
    "Unlock exclusive coupon deals for all orders",
  Unpaid: "غير مدفوعة",
  "Unsupported Format": "Formato no compatible",
  "Unsupported file format chosen":
    "Se eligió un formato de archivo no compatible",
  Update: " تحديث",
  "Update Address": "Actualizar dirección",
  "Update Payment": "Actualizar pago",
  "Update Payment Info": "Actualizar información de pago",
  "Update Profile": "Actualizar perfil",
  "Update To Cart": "التحديث إلى عربة التسوق",
  "Update Your Plan": "Actualiza tu plan",
  "Update To Cart": "التحديث إلى عربة التسوق",
  "Update Your Plan": "Actualiza tu plan",
  "Update to Cart": "Actualizar carrito",
  "Update to cart": "Actualizar carrito",
  Upload: "Subir",
  "Upload File": " رفع ملف ",
  "Upload Files": "Subir archivos",
  "Upload Prescription": "Subir receta",
  "Upload Prescriptions": "تحميل الوصفات",
  "Upload Prescriptions": "تحميل الوصفات",
  "Upload again": "Subir de nuevo",
  "Upload files first and they will appear here":
    "Sube archivos primero y aparecerán aquí",
  "Upload options -": "Opciones de subida -",
  "Upload your photo": "Sube tu foto",
  "Upload your prescription": "Sube tu receta",
  "Upload your prescriptions here and get your medicine right at your door step":
    "قم بتحميل الوصفات الطبية الخاصة بك هنا واحصل على الأدوية الخاصة بك في مكان قريب منك",
  Use: "Usa",
  "Use Code : ": "Usa el código: ",
  "Use Current Location": "انضم كرجل توصيل في",
  "Use Google's location service?": "هل تستخدم خدمة الموقع من Google؟",
  "Use your registered email/phone number to login":
    "Usa tu correo/teléfono registrado para iniciar sesión",
  "Use your registered phone number to get OTP":
    "Usa tu número registrado para obtener el OTP",
  User: "Usuario",
  "User Info": "Información del usuario",
  "User Name": "Nombre de usuario",
  VAT: "ضريبة القيمة المضافة",
  "VAT/TAX": "ضريبة القيمة المضافة",
  "VAT/TEX": "ضريبة القيمة المضافة",
  "VISIT US": "زورنا",
  "Valid from": "Válido desde",
  "Valid till": "Válido hasta",
  Variation: "تفاوت",
  "Vat/Tax": "IVA/Imp.",
  "Vat/Tax (Included)": "(ضريبة القيمة المضافة (مضمنة)",
  Veg: "نباتي",
  Vehicle: "Vehículo",
  "Vehicle Categories": "Categorías de vehículos",
  "Vehicle Details": "Detalles del vehículo",
  "Vehicle From This Vendor": "Vehículos de este proveedor",
  "Vehicle License Number": "Número de licencia del vehículo",
  "Vehicle List": "Lista de vehículos",
  Vehicles: "Vehículos",
  "Vehicles available": "Vehículos disponibles",
  "Vendor Application": "Solicitud de proveedor",
  "Vendor Info": "Información del proveedor",
  "Venture Starts Here": "La aventura comienza aquí",
  "Verified Pharmacies": "Farmacias verificadas",
  "Verified Provider": "Proveedor verificado",
  "Verified Providers": "Proveedores verificados",
  "Verified Restaurant": "Restaurante verificado",
  "Verified Store": "Tienda verificada",
  Verify: "يؤكد",
  "View All": "Ver todo",
  "View Cart": "Ver carrito",
  "View Cart List": "Ver lista del carrito",
  "View Details": "Ver detalles",
  "View Less": "Ver menos",
  "View More": "عرض المزيد",
  "View More": "عرض المزيد",
  "View My Orders": "Ver mis pedidos",
  "View Order Details": "Ver detalles del pedido",
  "View Saved Address": "عرض العنوان المحفوظ",
  "View all": "اظهار الكل",
  "Visit Again": "زورني مره اخرى",
  "Visit Again!": "زورني مره اخرى!",
  "View Saved Address": "عرض العنوان المحفوظ",
  "View all": "اظهار الكل",
  "Visit Again": "زورني مره اخرى",
  "Visit Again!": "زورني مره اخرى!",
  "Visit Store": "Visitar tienda",
  "Visit Vendor": "Visitar proveedor",
  "Visit store": "Visitar tienda",
  Wallet: "Cartera",
  "Wallet Amount": "مبلغ المحفظة",
  "Wallet Balance": "Saldo de la billetera",
  "Wallet Payment": "Pago con billetera",
  "Wallet balance is below total amount": "رصيد المحفظة أقل من المبلغ الإجمالي",
  "Wallet balance is below total amount.":
    "El saldo de la billetera es menor al monto total.",
  "Wallet payment": "دفع المحفظة",
  Wallets: "محافظ",
  "Wanna Try  Again!": "هل تريد المحاولة مرة أخرى!",
  "Wanna Try  Again!!": "¿Quieres intentar de nuevo?",
  "Want to login to your services ?": " هل تريد تسجيل الدخول إلى خدماتك؟",
  "Want to pay partially with wallet?":
    "¿Quieres pagar parcialmente con la billetera?",
  "Want to pay via your wallet?": "¿Quieres pagar con tu billetera?",
  "Watch trending items and shop instantly":
    "Mira los artículos populares y compra al instante",
  "Watch, choose, and book your ride": "Mira, elige y reserva tu viaje",
  We: "نحن",
  "We Deliver Everything": "Entregamos todo",
  "We are under maintainance.": "نحن تحت الصيانة.",
  "We are under maintenance.": "نحن تحت الصيانة.",
  "We could not find any active benefits on your plan right now.":
    "No hemos encontrado beneficios activos en tu plan en este momento.",
  "We found": "وجدنا",
  "We provide best quality & fresh grocery items near your location":
    "Ofrecemos comestibles frescos y de la mejor calidad cerca de tu ubicación",
  "We provide best quality & valuable products around the world":
    "نحن نقدم أفضل جودة ومنتجات قيمة حول العالم",
  "We will be back very soon.": "ونحن سوف يعود قريبا جدا.",
  "We will begin processing your order shortly. Your Order ID is":
    "Comenzaremos a procesar tu pedido en breve. Tu ID de pedido es",
  "Welcome to ! {{name}}": "Welcome to ! {{name}}",
  "Welcome to 6amMart!": "¡Bienvenido a 6amMart!",
  "Welcome to React": "Bienvenido a React",
  "Welcome to {{name}}": "Welcome to {{name}}",
  "We’re just a click away if you have more questions.":
    "Estamos a un clic de distancia si tienes más preguntas.",
  "We’ve sent a verification code to {{contact}}":
    "We’ve sent a verification code to {{contact}}",
  "What are the parcel size limits?":
    "¿Cuáles son los límites de tamaño del paquete?",
  "What are the payment options?": "¿Cuáles son las opciones de pago?",
  "What are you sending ?": "ماذا ترسل",
  "What are you wish to send?": "¿Qué deseas enviar?",
  "What everyone's watching and ordering":
    "Lo que todos están viendo y pidiendo",
  "What is parcel delivery?": "¿Qué es la entrega de paquetes?",
  "What is the rental service?": "¿Qué es el servicio de alquiler?",
  "What items can I send?": "¿Qué artículos puedo enviar?",
  "What vehicle types are available?":
    "¿Qué tipos de vehículos están disponibles?",
  "What's on discount right now?": "¿Qué está en descuento ahora?",
  "What's trending right now?": "¿Qué está de moda ahora?",
  "Whats New": "Novedades",
  "What’s Wrong With This Order?": "ما الخطأ في هذا الطلب؟",
  "What’s Wrong With This Order?": "ما الخطأ في هذا الطلب؟",
  "What’s Wrong With This Order? ": "¿Qué está mal con este pedido? ",
  "When you enable monthly reorder:": "Cuando activas el pedido mensual:",
  "When you enable monthly reorder:": "Cuando activas el pedido mensual:",
  "Where To Go?": "¿A dónde ir?",
  "Where to deliver?": "¿A dónde entregar?",
  "Who Will Pay?": "¿Quién pagará?",
  "Why upload prescription -": "Por qué subir receta -",
  WishList: "قائمة الرغبات",
  "Without any payment method, you can not place the order.":
    "بدون أي طريقة دفع ، لا يمكنك تقديم الطلب",
  Yes: " نعم ",
  "Yes, Cancel": "Sí, cancelar",
  "Yes, Delete": " نعم ، احذف ",
  "Yes, Remove": "Yes, Remove",
  "Yes, Reorder": "Sí, Reordenar",
  "Yes,Cancel": "Sí, cancelar",
  "Yes,Received": "Sí, recibido",
  Yesterday: "Ayer",
  "You Have Earned Point": "Has ganado puntos",
  "You Sent": "You Sent",
  "You are not logged in": "لم يتم دخولك",
  "You can have it delivered now or pick a time for scheduled delivery!":
    "¡Puedes recibirlo ahora o elegir una hora para entrega programada!",
  "You can not Order more then 1000৳ on COD order":
    "لا يمكنك طلب أكثر من 1000 عند طلب الدفع عند الاستلام",
  "You can pay the full amount with your wallet.":
    "Puedes pagar el monto total con tu billetera.",
  "You can upload a maximum of 5 prescriptions":
    "Puedes subir un máximo de 5 recetas",
  "You can upload up to": "Puedes subir hasta",
  "You can use this ID to track your order later":
    "Puedes usar este ID para rastrear tu pedido más tarde",
  "You can't add more than the allowed quantity of this item":
    "No puedes agregar más de la cantidad permitida de este artículo",
  "You cancelled the subscription payment. No charge was made.":
    "Cancelaste el pago de la suscripción. No se realizó ningún cargo.",
  "You cannot order from multiple restaurants in a single order":
    "لا يمكنك الطلب من عدة مطاعم في طلب واحد",
  "You denied location permission. Please allow browsers location permission from your device, refresh the site and receive more accurate delivery.":
    "لقد رفضت إذن تحديد الموقع. يرجى السماح بإذن تحديد موقع المستعرضات من جهازك ، وتحديث الموقع والحصول على توصيل أكثر دقة.",
  "You do not have sufficient balance to pay full amount via wallet.":
    "No tienes saldo suficiente para pagar el monto total con la billetera.",
  "You got a free home delivery service":
    "Obtuviste un servicio de entrega a domicilio gratis",
  "You have Saved": "لقد قمت بحفظ",
  "You have balance in  your wallet": "Tienes saldo en tu billetera",
  "You have earned": "لقد ربحت نقطة",
  "You have food from another restaurant in cart. If you continue, your all previous food from cart will be removed":
    "لديك طعام من مطعم آخر في عربة التسوق. إذا تابعت ، ستتم إزالة كل طعامك السابق من عربة التسوق",
  "You have item from another store in cart. If you continue, your all previous item from cart will be removed":
    "Tienes artículos de otra tienda en el carrito. Si continúas, todos los artículos anteriores serán eliminados.",
  "You have item from another store in cart. If you continue, your all previous item from cart will be removed and this one will be added.":
    "لديك عنصر من متجر آخر في سلة التسوق. إذا تابعت ، ستتم إزالة جميع العناصر السابقة من سلة التسوق وستتم إضافة هذا العنصر.",
  "You have no channels.": "ليس لديك قنوات.",
  "You have not added any address yet.":
    "Aún no has agregado ninguna dirección.",
  "You have nothing in your cart to checkout.":
    "No tienes nada en tu carrito para pagar.",
  "You have reached your maximum limit of": "Has alcanzado tu límite máximo de",
  "You have reduced delivery charge": "Tienes un cargo de entrega reducido",
  "You have to add min": "Debes agregar mín.",
  "You just reached the destination, please make payment to complete the trip":
    "Acabas de llegar al destino, por favor realiza el pago para completar el viaje",
  "You made your earning when the order is complete":
    "Recibes tus ganancias cuando el pedido se complete",
  "You must accept the": "يجب عليك قبول",
  "You must accept the terms and conditions":
    "Debes aceptar los términos y condiciones",
  "You must be login to access this page.":
    "Debes iniciar sesión para acceder a esta página.",
  "You must pick a zone to access this page.":
    "Debes elegir una zona para acceder a esta página.",
  "You place the order successfully.": "تهانينا! لقد قمت بوضع الطلب بنجاح",
  "You save": "You save",
  "You saved": "Ahorraste",
  "You will  pay return fee": "Pagarás la tarifa de devolución",
  "You will receive a remainder notification before next order":
    "Recibirás una notificación de recordatorio antes del próximo pedido",
  "You won’t be able to recover your data again":
    "No podrás recuperar tus datos nuevamente",
  "You're about to switch to": "Estás a punto de cambiar a",
  "You're free from the fear of losing prescriptions, find your digital prescription with Lifetime Cure.":
    "Estás libre del miedo de perder recetas, encuentra tu receta digital con Lifetime Cure.",
  Your: "Tu",
  "Your Address": "عنوانك",
  "Your Cart is Waiting!": "¡Tu carrito está esperando!",
  "Your Email": "Tu correo",
  "Your Email Address": "عنوان بريدك  الإلكتروني",
  "Your Favourites": "مفضلاتك",
  "Your Health, Delivered Fast": "Tu Salud, Entregada Rápido",
  "Your Email Address": "عنوان بريدك  الإلكتروني",
  "Your Favourites": "مفضلاتك",
  "Your Health, Delivered Fast": "Tu Salud, Entregada Rápido",
  "Your Last Orders": "Tus últimos pedidos",
  "Your Last Parcels": "Tus Últimos Paquetes",
  "Your Last Parcels": "Tus Últimos Paquetes",
  "Your Last Trips": "Tus últimos viajes",
  "Your Location": "Your Location",
  "Your Location": "Your Location",
  "Your Payment ": "Tu pago ",
  "Your Payment was": "Tu pago fue",
  "Your Pro subscription has expired. Renew to continue enjoying exclusive benefits.":
    "Tu suscripción Pro ha caducado. Renueva para seguir disfrutando de beneficios exclusivos.",
  "Your Pro subscription is now active. Enjoy your benefits!":
    "Tu suscripción Pro está activa. ¡Disfruta de tus beneficios!",
  "Your Profile": "ملفك الشخصي",
  "Your Referral Code": "رمز الإحالة الخاص بك",
  "Your Registration Could Not Completed .": "Tu registro no pudo completarse.",
  "Your Registration Has been Completed Successfully.":
    "Tu registro se completó con éxito.",
  "Your Trip has been ": "Tu viaje ha sido ",
  "Your Trip is": "Tu viaje está",
  "Your Wishlist": "قائمة امنياتك",
  "Your account is inactive or Your token has been expired":
    "Tu cuenta está inactiva o tu token ha expirado",
  "Your browser location track permission is off. Please turn on the location permission to detect current location":
    "El permiso de seguimiento de ubicación del navegador está desactivado. Actívalo para detectar la ubicación actual",
  "Your cart has been cleared as the selected zone does not support the previous pickup point.":
    "Tu carrito se ha vaciado ya que la zona seleccionada no soporta el punto de recogida anterior.",
  "Your cart has been cleared successfully. Now try to add the product again.":
    "Tu carrito se ha vaciado con éxito. Ahora intenta agregar el producto de nuevo.",
  "Your cart is empty": "Tu carrito está vacío",
  "Your current Pro plan does not include any savings for this module. Switch plans to unlock more rewards.":
    "Tu plan Pro actual no incluye ahorros para este módulo. Cambia de plan para desbloquear más recompensas.",
  "Your food will be delivered within": "سيتم توصيل طعامك في الداخل",
  "Your loyalty point will convert to currency and transfer to wallet":
    "سيتم تحويل نقاط الولاء الخاصة بك إلى عملة وتحويلها إلى المحفظة",
  "Your order is": "Tu pedido está",
  "Your order is placed Successfully. We start our delivery process and you will receive your food soon":
    "تم تقديم طلبك بنجاح. نبدأ عملية التوصيل وستتلقى طعامك قريبًا",
  "Your order is placed Successfully. We start our delivery process and you will receive your food soon.":
    "تم تقديم طلبك بنجاح. نبدأ عملية التوصيل وستتلقى طعامك قريبًا",
  "Your order is placed but": "Tu pedido fue realizado pero",
  "Your order will not be placed automatically without your confirmation":
    "Tu pedido no se realizará automáticamente sin tu confirmación",
  "Your parcel request submitted successfully! to check your parcel status please track order.":
    "¡Tu solicitud de paquete se envió con éxito! Para verificar el estado de tu paquete, rastrea el pedido.",
  "Your payment has been successfully processed, and your order ":
    "Tu pago se procesó con éxito, y tu pedido ",
  "Your payment was incomplete. Please choose an option below to complete your transaction.":
    "Tu pago estuvo incompleto. Por favor elige una opción abajo para completar tu transacción.",
  "Your payment was not successful. Please try again to complete the trip":
    "Tu pago no fue exitoso. Por favor intenta de nuevo para completar el viaje",
  "Your plan is about to expire. Renew now to continue enjoying exclusive benefits and savings.":
    "Tu plan está a punto de caducar. Renueva ahora para seguir disfrutando de beneficios y ahorros exclusivos.",
  "Your provided tips will 100% goes to deliveryman.":
    "Tus propinas irán 100% al repartidor.",
  "Your request has been submitted successfully. your preferable car will be arrived on time after confirmation.":
    "Tu solicitud se envió con éxito. Tu auto preferido llegará a tiempo después de la confirmación.",
  "Your request has been submitted successfully. your preferable vehicles will be arrived on time after confirmation.":
    "Tu solicitud se envió con éxito. Tus vehículos preferidos llegarán a tiempo después de la confirmación.",
  "Your restaurant URL has been copied":
    "La URL de tu restaurante se ha copiado",
  "Your selected items will be automatically added to your cart every month.":
    "Los artículos seleccionados se agregarán automáticamente a tu carrito cada mes.",
  "Your store URL has been copied": "La URL de tu tienda se ha copiado",
  "Your subscription payment did not go through. Please try again.":
    "El pago de tu suscripción no se completó. Por favor, inténtalo de nuevo.",
  "Your token has been expired.please sign in again":
    "انتهت صلاحية الرمز المميز الخاص بك. يرجى تسجيل الدخول مرة أخرى",
  "Your total price must be more then coupon amount":
    "Tu precio total debe ser mayor al monto del cupón",
  "Your trip is": "Tu viaje está",
  "Your trip is canceled": "Tu viaje fue cancelado",
  "Your trip is confirmed": "Tu viaje está confirmado",
  "Your wallet balance is insufficient for partial payment.":
    "El saldo de tu billetera es insuficiente para el pago parcial.",
  "Your wallet balance is insufficient for payment.":
    "El saldo de tu billetera es insuficiente para el pago.",
  "Your {{plan}} has expired. Renew your subscription to continue enjoying exclusive benefits.":
    "Tu {{plan}} ha caducado. Renueva tu suscripción para seguir disfrutando de beneficios exclusivos.",
  "You’ll reach your destination approximately in":
    "Llegarás a tu destino aproximadamente en",
  "Z to A": "من ي إلى أ",
  Zone: "منطقة",
  "Zone Name": "اسم المنطقة ",
  ac: "ac",
  accepted: "وافقت",
  add_fund_by_admin: "إضافة تمويل من قبل المشرف",
  "added via": "agregado vía",
  "added via ": "agregado vía ",
  admin: "مشرف",
  all: "الجميع",
  "amount in wallet": "monto en la billetera",
  and: "y",
  "and Maximum": "والحد الأقصى",
  "and bad weather charge": "y cargo por mal clima",
  "and maximum": "y máximo",
  "and special saving for": "y ahorro especial para",
  "and special saving for mart pro members":
    "y ahorro especial para miembros Pro",
  "as a Pro member.": "como miembro Pro.",
  "ascending (A-Z)": "تصاعدي (أ-ي)",
  away: "بعيد",
  bites: "bocados",
  bonus: "bono",
  "can not remain without selection.": "لا يمكن أن يبقى بدون اختيار",
  canceled: "ألغيت",
  "cash on delivery": "الدفع عند الاستلام",
  cash_on_delivery: "الدفع عند الاستلام",
  "cashback. The minimum purchase required to avail this offer is":
    "reembolso. La compra mínima requerida para esta oferta es",
  "clear all filter": "مسح كل عامل التصفية",
  completed: "completado",
  confirmed: "تم تأكيد",
  coupons: "cupones",
  credit: "تنسب إليه",
  days: "días",
  debit: "مدين",
  delivered: "تم التوصيل",
  delivery: "توصيل",
  "delivery man": "رجل التوصيل",
  "delivery man info": "información del repartidor",
  "descending (Z-A)": "تنازلي (Z-A)",
  discount: "descuento",
  "discount is applicable.": "el descuento es aplicable.",
  "discount will be applicable when  order amount exceeds is more than":
    "el descuento será aplicable cuando el monto del pedido exceda",
  "distance wise": "por distancia",
  distance_wise: "por distancia",
  "e-Commerce !": "¡e-Commerce!",
  eBusiness: "الأعمال الإلكترونية",
  "exceeds the maximum file size": "excede el tamaño máximo del archivo",
  "field is empty": "الحقل فارغ",
  files: "archivos",
  food: "غذاء",
  "food for you": " أطعمة لك ",
  foods: "الأطعمة",
  "foods for you": "أطعمة لك",
  "for Chinese, Japanies & Asian foods": "للأطعمة الصينية واليابانية والآسيوية",
  "for every referral!": "¡por cada referido!",
  "for the first time, make sure to use the referral code":
    "por primera vez, asegúrate de usar el código de referido",
  "for you": "لك",
  from: "de",
  "from order details.": "desde los detalles del pedido.",
  "from trip details.": "desde los detalles del viaje.",
  "from you": "de ti",
  "fund to get max of": "fondos para obtener máx. de",
  get: "obtener",
  handover: "سلم",
  "has been copied": "تم نسخه",
  "has been placed.": "ha sido realizado.",
  help: "ayuda",
  here: "aquí",
  home: "مسكن",
  hourly: "por hora",
  "in your cart": "في عربة التسوق الخاصة بك",
  inbox: "bandeja de entrada",
  incomplete: "incompleto",
  "insufficient amount": "كمية غير كافية",
  "is not available right now.": "غير متوفر الآن.",
  "is not available right now. You can order it later.":
    "غير متوفر الآن. يمكنك طلبها لاحقًا.",
  "is out of stock": "está agotado",
  "is unavailable": "no está disponible",
  items: "أغراض",
  "items for you !": "عناصر لك!",
  "items in your cart": "الوحدات الموجودة فى سلة التسوق الخاصة بك",
  "items selected": " العناصر المحددة ",
  "items.": "العناصر.",
  km: "km",
  "last name required": "نشاط اسم مطلوب",
  "loyalty points": "puntos de lealtad",
  loyalty_point: "نقطة ولاء",
  map: "mapa",
  "map-image": "imagen del mapa",
  max: "máx.",
  min: "دقيقة",
  "min Order of": "pedido mín. de",
  mins: "min",
  "more for free delivery": "más para entrega gratis",
  "more to save with Pro Plan": "more to save with Pro Plan",
  "must be in between Minimum": "يجب أن يكون بين الحد الأدنى",
  "more to save with Pro Plan": "more to save with Pro Plan",
  "must be in between Minimum": "يجب أن يكون بين الحد الأدنى",
  "my orders": "mis pedidos",
  "nagad_number is required": "se requiere nagad_number",
  "name is required": "se requiere nombre",
  "non ac": "sin ac",
  "non-veg": "غير نباتي",
  "non-veg": "غير نباتي",
  "not now": "ahora no",
  off: "de descuento",
  "off as a Pro member": "off as a Pro member",
  "off delivery as a Pro member": "off delivery as a Pro member",
  "off on all orders": "de descuento en todos los pedidos",
  "off on delivery fee": "de descuento en gastos de envío",
  "off on delivery fee as a Pro member": "off on delivery fee as a Pro member",
  office: "مكتب",
  "offline payment": "pago offline",
  on: "en",
  "on deliveries above": "on deliveries above",
  "on delivery fees as a Pro member.": "on delivery fees as a Pro member.",
  "on joining": "عند الانضمام",
  "on orders above": "en pedidos superiores a",
  ongoing: "en curso",
  "or login with": "o inicia sesión con",
  order: "pedido",
  "order ID and phone number": "ID de pedido y número de teléfono",
  "order placed": "pedido realizado",
  "order refund": "طلب استرداد",
  "order refund": "طلب استرداد",
  "order-summary": "Resumen del pedido",
  order_place: "مكان الطلب",
  others: "آحرون",
  "out of limits": "fuera de límites",
  out_of_limits: "خارج الحدود",
  "partially paid amount": "monto pagado parcialmente",
  "payment_by is required": "se requiere payment_by",
  "pdf, doc, jpg. File size : max 2 MB": "pdf, doc, jpg. Tamaño máx.: 2 MB",
  pending: "ريثما",
  "phone number required": "رقم الهاتف مطلوب",
  picked_up: "التقط",
  "placed using the phone number": "realizado usando el número de teléfono",
  "please login first": "por favor inicia sesión primero",
  point: "نقطة",
  "point.": "نقطة",
  point_to_wallet: "أشر إلى المحفظة",
  points: "نقاط",
  popular_stores: "المتاجر الشعبية",
  "prescription is required": "se requiere receta",
  prescriptions: "recetas",
  previous: "anterior",
  processing: "يعالج",
  "profile settings": "configuración del perfil",
  "quantity available": "cantidad disponible",
  referral: "الإحالة",
  "referral code": "código de referido",
  "referral-code": "código de referido",
  referrer: "المرجع",
  "refresh ": "actualizar ",
  "refund requested": "استرداد طلب",
  "refund requested canceled": "تم إلغاء طلب رد الأموال",
  refund_requested: "استرداد طلب",
  refunded: "برد",
  "restaurant address required": "عنوان المطعم مطلوب",
  "restaurant name required": "اسم المطعم مطلوب",
  restaurants: " مطاعم ",
  satisfied: "راضي",
  selected: "seleccionado",
  "seller info": "información del vendedor",
  sender: "remitente",
  settings: "configuración",
  "some Customer & Restaurant Owners": "بعض العملاء وأصحاب المطاعم",
  "something went wrong.": "هناك خطأ ما.",
  "start from": "comenzar desde",
  store_registration_commission_description:
    "descripción de la comisión de registro de tienda",
  stores: "tiendas",
  "subscription plan": "subscription plan",
  "take away": "يبعد",
  take_away: "يبعد",
  "terms and conditions": "الأحكام والشروط",
  "this page or go back to previous page":
    "esta página o vuelve a la página anterior",
  to: "a",
  "to get extra savings in every order.":
    "para obtener ahorros adicionales en cada pedido.",
  "to get free delivery": "para obtener envío gratis",
  "to start your business": "لبدء عملك",
  "toggle password visibility": "alternar visibilidad de contraseña",
  "track order": "rastrear pedido",
  "up to": "hasta",
  upto: "upto",
  "using your code and place their first order":
    "usando tu código y realizan su primer pedido",
  "vat is required": "ضريبة القيمة المضافة مطلوبة",
  veg: "نباتي",
  vendor: "بائع",
  via: "via",
  view: "رأي",
  wallet: "billetera",
  warning: "advertencia",
  "when you sign up. It's my way of welcoming you to this awesome e-commerce platform! Happy shopping on":
    "cuando te registres. Es mi forma de darte la bienvenida a esta increíble plataforma de e-commerce. ¡Feliz compra en",
  "where you want to sent your password recovery OTP.":
    "a donde deseas que se envíe el OTP de recuperación de contraseña.",
  "will be added to your cart. Your existing cart may be replaced.":
    "will be added to your cart. Your existing cart may be replaced.",
  "will be applied.": "will be applied.",
  "with Pro Plan": "with Pro Plan",
  "with a": "with a",
  "with a Pro coupon.": "with a Pro coupon.",
  "with affordable price.": "con un precio asequible.",
  "write something": "أكتب شيئا",
  "write something": "أكتب شيئا",
  "you can rely on.": "en la que puedes confiar.",
  "{{percent}}% off as a Pro member":
    "{{percent}}% de descuento como miembro Pro",
  "{{percent}}% off as a Pro member (up to {{cap}})":
    "{{percent}}% de descuento como miembro Pro (hasta {{cap}})",
  "{{percent}}% off as a Pro member (up to {{cap}}) on orders above {{amount}}":
    "{{percent}}% de descuento como miembro Pro (hasta {{cap}}) en pedidos superiores a {{amount}}",
  "{{percent}}% off as a Pro member on orders above {{amount}}":
    "{{percent}}% de descuento como miembro Pro en pedidos superiores a {{amount}}",
  "{{percent}}% off on delivery fee as a Pro member":
    "{{percent}}% de descuento en gastos de envío como miembro Pro",
  "{{percent}}% off on delivery fee as a Pro member on deliveries above {{amount}}":
    "{{percent}}% de descuento en gastos de envío como miembro Pro en entregas superiores a {{amount}}",
  "{{percent}}% off on delivery fee as a Pro member on orders above {{amount}}":
    "{{percent}}% de descuento en gastos de envío como miembro Pro en pedidos superiores a {{amount}}",
  "Book Now": "Book Now",
  "Offline Payment Information": "Offline Payment Information",
  "Switch Module": "Switch Module",
  "The category you want to view is not in your currently selected module. Switch the module to view this category.":
    "The category you want to view is not in your currently selected module. Switch the module to view this category.",
  "The item you want to view is not in your currently selected module. Switch the module to view this item.":
    "The item you want to view is not in your currently selected module. Switch the module to view this item.",
  "The store you want to view is not in your currently selected module. Switch the module to view this store.":
    "The store you want to view is not in your currently selected module. Switch the module to view this store.",
  Unavailable: "Unavailable",
  item: "item",
  "1 date was skipped because the selected time has already passed today.":
    "1 date was skipped because the selected time has already passed today.",
  "1 occurrence was skipped because the selected time has already passed.":
    "1 occurrence was skipped because the selected time has already passed.",
  "Address type": "Address type",
  "Adjust quantities or remove variations from cart":
    "Adjust quantities or remove variations from cart",
  "All bids have been reviewed. Check back later for new offers.":
    "All bids have been reviewed. Check back later for new offers.",
  "Also Booked Together": "Also Booked Together",
  Approve: "Approve",
  "Are you sure you want to cancel this booking?":
    "Are you sure you want to cancel this booking?",
  "Are you sure you want to cancel this scheduled service":
    "Are you sure you want to cancel this scheduled service",
  "Are you sure you want to delete this service?":
    "Are you sure you want to delete this service?",
  "Arrival Time": "Arrival Time",
  "Back To Main Booking": "Back To Main Booking",
  "Back to Main Orders": "Volver a los pedidos principales",
  "Bid rejected successfully": "Bid rejected successfully",
  "Bid selected successfully": "Bid selected successfully",
  Bidding: "Bidding",
  "Billing Summary": "Resumen de facturación",
  Booking: "Booking",
  "Booking Date:": "Booking Date:",
  "Booking ID:": "Booking ID:",
  "Booking Id": "Booking Id",
  "Booking cancelled successfully": "Booking cancelled successfully",
  "Booking placed successfully": "Booking placed successfully",
  "Browse by Categories": "Browse by Categories",
  "By continuing, you will agree with": "By continuing, you will agree with",
  "By selecting this service will get from provider location":
    "By selecting this service will get from provider location",
  "By selecting this service will provided at your location":
    "By selecting this service will provided at your location",
  "Can't find the service you need? Submit a request and help us improve our service offerings.":
    "Can't find the service you need? Submit a request and help us improve our service offerings.",
  "Cancel Booking": "Cancel Booking",
  "Cancel Post": "Cancel Post",
  "Cancel Request": "Cancel Request",
  "Cash After Service": "Cash After Service",
  "Category is required": "Category is required",
  "Check this to repeat selected days in a date range":
    "Check this to repeat selected days in a date range",
  "Choose Booking Type": "Choose Booking Type",
  "Choose when you want get your service":
    "Choose when you want get your service",
  "Choose when you want to get your service on selected days.":
    "Choose when you want to get your service on selected days.",
  "Choose which days you want your service":
    "Choose which days you want your service",
  "Choose which variation service you want":
    "Choose which variation service you want",
  "Clear All Items?": "Clear All Items?",
  "Confirm Dates": "Confirm Dates",
  "Confirm Days": "Confirm Days",
  "Confirm Variation": "Confirm Variation",
  "Create New": "Create New",
  "Create a request with your own description and additional instructions.":
    "Create a request with your own description and additional instructions.",
  "Create a service request, share your requirements, and receive offers from qualified providers ready to help.":
    "Create a service request, share your requirements, and receive offers from qualified providers ready to help.",
  "Custom Schedule Service": "Custom Schedule Service",
  "Custom Service": "Custom Service",
  "Custom Service Created Successfully!":
    "Custom Service Created Successfully!",
  "Custom Service Details": "Custom Service Details",
  "Custom Service List": "Custom Service List",
  Customer: "Customer",
  "Customer Details": "Customer Details",
  Date: "Date",
  "Date Range": "Date Range",
  Day: "Day",
  "Day Wise Arrival Time": "Day Wise Arrival Time",
  "Delete Request": "Delete Request",
  "Description is required": "Description is required",
  "Download Invoice": "Download Invoice",
  "Downloading invoice...": "Downloading invoice...",
  "Edit Service Request": "Edit Service Request",
  "Enter a description...": "Enter a description...",
  "Enter a service description...": "Enter a service description...",
  "Enter your booking ID and phone number to get live updates":
    "Enter your booking ID and phone number to get live updates",
  "Enter your booking id": "Enter your booking id",
  "Est. Service Time": "Est. Service Time",
  "Ex: 06:00 PM": "Ex: 06:00 PM",
  "Ex: 20 Jul 26 – 20 Aug 26": "Ex: 20 Jul 26 – 20 Aug 26",
  "Ex: Car Wash": "Ex: Car Wash",
  "Ex: Saturday, Sunday": "Ex: Saturday, Sunday",
  Expired: "Expired",
  "Explore Our Services": "Explore Our Services",
  "Featured Providers": "Featured Providers",
  Feedback: "Feedback",
  "Find the Right Provider for Your Service":
    "Find the Right Provider for Your Service",
  "Get Service At": "Get Service At",
  "Get fastest service from your nearby Service Provides":
    "Get fastest service from your nearby Service Provides",
  ID: "ID",
  "Instant Service": "Instant Service",
  "Instant service is not available. Please select a future time":
    "Instant service is not available. Please select a future time",
  "Invoice downloaded": "Invoice downloaded",
  Loading: "Loading",
  "Make Request": "Make Request",
  "Min Booking": "Min Booking",
  "Most Popular Services": "Most Popular Services",
  "Most trusted services chosen by thousands of users.":
    "Most trusted services chosen by thousands of users.",
  "Multiple Booking Type": "Multiple Booking Type",
  "My Details": "My Details",
  "My Location": "My Location",
  "My Requested Services": "My Requested Services",
  "Need a Service? Create a Request": "Need a Service? Create a Request",
  "New Service Request": "New Service Request",
  "Next Service": "Next Service",
  "No Bids Yet": "No Bids Yet",
  "No campaigns available": "No campaigns available",
  "No changes to update": "No changes to update",
  "No service could be added to cart": "No service could be added to cart",
  "No service man assigned": "No service man assigned",
  "No services available": "No services available",
  "No sub category found": "No sub category found",
  "Offer Price": "Offer Price",
  Ongoing: "Ongoing",
  "Ongoing Service": "Ongoing Service",
  "Order Date:": "Fecha del pedido:",
  "Order Note": "Order Note",
  "Parcel Received": "Parcel Received",
  "Placing...": "Placing...",
  "Please give a rating": "Please give a rating",
  "Please log in to use Multiple Booking":
    "Please log in to use Multiple Booking",
  "Please select a date": "Please select a date",
  "Please select a future date and time":
    "Please select a future date and time",
  "Please select a service address": "Please select a service address",
  "Please select a time": "Please select a time",
  "Please select a variation": "Please select a variation",
  "Please select an address type": "Please select an address type",
  "Please set arrival time for all selected dates":
    "Please set arrival time for all selected dates",
  "Please update the arrival time to resolve the conflict before confirming.":
    "Please update the arrival time to resolve the conflict before confirming.",
  "Post for Customised Service": "Post for Customised Service",
  "Preferable Date": "Preferable Date",
  "Privacy Policy,": "Privacy Policy,",
  "Processing...": "Processing...",
  "Product Video": "Product Video",
  "Product video preview": "Product video preview",
  Provider: "Provider",
  "Provider List": "Provider List",
  "Provider Location": "Provider Location",
  "Provider is not available at the selected time on":
    "Provider is not available at the selected time on",
  "Quick & Emergency Experts": "Quick & Emergency Experts",
  "Rate the serviceman": "Rate the serviceman",
  "Rate this service": "Rate this service",
  "Recommended Services Nearby": "Recommended Services Nearby",
  "Reordering from {{name}} will clear your current provider cart. Do you want to continue?":
    "Reordering from {{name}} will clear your current provider cart. Do you want to continue?",
  "Repeat This Every Week": "Repeat This Every Week",
  "Repeat This Service": "Repeat This Service",
  "Request cancelled successfully": "Request cancelled successfully",
  "Request deleted successfully": "Request deleted successfully",
  "Request posted successfully": "Request posted successfully",
  "Request updated successfully": "Request updated successfully",
  "Requested Service": "Requested Service",
  "Requested Service Name": "Requested Service Name",
  "Requested Services": "Requested Services",
  "Review submitted": "Review submitted",
  "Schedule Days": "Schedule Days",
  "Schedule Service": "Schedule Service",
  "Search Booking": "Search Booking",
  "Select Category": "Select Category",
  "Select Date & Time": "Select Date & Time",
  "Select Date and Time": "Select Date and Time",
  "Select Days": "Select Days",
  "Select Sub Category": "Select Sub Category",
  "Select Variation": "Select Variation",
  "Select a service category, describe your requirements, and receive offers from qualified providers.":
    "Select a service category, describe your requirements, and receive offers from qualified providers.",
  "Select dates from the calendar": "Select dates from the calendar",
  "Selected Date & time": "Selected Date & time",
  "Selected Days & Time": "Selected Days & Time",
  "Send Request": "Send Request",
  Service: "Service",
  "Service Address": "Service Address",
  "Service Date:": "Service Date:",
  "Service Description": "Service Description",
  "Service Details": "Service Details",
  "Service Name": "Service Name",
  "Service Provided at": "Service Provided at",
  "Service Providers": "Service Providers",
  "Service Time": "Service Time",
  "Service updated!": "Service updated!",
  Servicemen: "Servicemen",
  "Services added to cart": "Services added to cart",
  "Some selected date-times are in the past. Please choose a future time.":
    "Some selected date-times are in the past. Please choose a future time.",
  "Start a New Booking?": "Start a New Booking?",
  "Sub Booking ID:": "Sub Booking ID:",
  "Sub category is required": "Sub category is required",
  "Submit Custom Service": "Submit Custom Service",
  "Switch to Cash After Service": "Switch to Cash After Service",
  "Tell Us More About Your Desired Service":
    "Tell Us More About Your Desired Service",
  "Terms & Condition": "Terms & Condition",
  "The content will be translated to the selected language.":
    "The content will be translated to the selected language.",
  "This belongs to another module. Switch now to view it?":
    "This belongs to another module. Switch now to view it?",
  "This booking will be continued": "This booking will be continued",
  "This provider has no available schedule":
    "This provider has no available schedule",
  "This will clear your entire cart from every store. This action cannot be undone.":
    "This will clear your entire cart from every store. This action cannot be undone.",
  "Top Providers": "Top Providers",
  "Track Your Booking": "Track Your Booking",
  Upcoming: "Upcoming",
  "Update Cart": "Update Cart",
  "Update Custom Service": "Update Custom Service",
  "Update Request": "Update Request",
  "Update Variation": "Update Variation",
  "Variation Available": "Variation Available",
  "Variation Selected": "Variation Selected",
  "Variation {{num}}": "Variation {{num}}",
  Variations: "Variations",
  Video: "Video",
  View: "View",
  "View Post": "View Post",
  "View Service Time": "View Service Time",
  "Yes, Clear All": "Yes, Clear All",
  "Yes, Rebook": "Yes, Rebook",
  "You can get the service now or schedule it for a later time!":
    "You can get the service now or schedule it for a later time!",
  "You can have the service provided now or pick a time for scheduled service provide!":
    "You can have the service provided now or pick a time for scheduled service provide!",
  "You have to go to provider location in order to receive this service":
    "You have to go to provider location in order to receive this service",
  "You will receive this services in the upcoming":
    "You will receive this services in the upcoming",
  "You will receive this services total":
    "You will receive this services total",
  "Your Previous Booking": "Your Previous Booking",
  "Your custom service has been created successfully. Providers can view your custom service and send offers based on your requirements.":
    "Your custom service has been created successfully. Providers can view your custom service and send offers based on your requirements.",
  "Your provider URL has been copied": "Your provider URL has been copied",
  "Your service booking date": "Your service booking date",
  "bookings placed successfully": "bookings placed successfully",
  "dates were skipped because the selected time has already passed.":
    "dates were skipped because the selected time has already passed.",
  "decrease quantity": "decrease quantity",
  image: "image",
  "increase quantity": "increase quantity",
  "occurrences were skipped because the selected time has already passed.":
    "occurrences were skipped because the selected time has already passed.",
  times: "times",
  "week days.": "week days.",
  "Service Completed": "Service Completed",
  "At Customer's Location": "At Customer's Location",
  "At Provider's Location": "At Provider's Location",
  "Home Service": "Home Service",
  "Visit Provider": "Visit Provider",
  "Please visit provider location": "Please visit provider location",
  "Provider will come to your location": "Provider will come to your location",
  Canceled: "Canceled",
  Cancelled: "Cancelled",
  "Per booking": "Per booking",
  "Total for": "Total for",
  Validity: "Validity",
  "View single booking": "View single booking",
  "View total": "View total",
  bookings: "bookings",
  "Single booking": "Single booking",
  "Coupon removed. Please re-apply after updating your booking.":
    "Coupon removed. Please re-apply after updating your booking.",
  FAQ: "FAQ",
  "Frequently Bought Together": "Frequently Bought Together",
  "Max Service Time": "Max Service Time",
  "Maximum Service Time": "Maximum Service Time",
  "Min Service Time": "Min Service Time",
  "Minimum Service Time": "Minimum Service Time",
  "Please provide a reason": "Please provide a reason",
  "Reason for Cancellation": "Reason for Cancellation",
  "Tell us why you're cancelling this booking":
    "Tell us why you're cancelling this booking",
  Booked: "Booked",
  "Visit provider": "Visit provider",

  "{{percent}}% off on {{module}} (up to {{cap}}) (on orders above {{min}})":
    "{{percent}}% off on {{module}} (up to {{cap}}) (on orders above {{min}})",
  "{{percent}}% off on {{module}} (up to {{cap}})":
    "{{percent}}% off on {{module}} (up to {{cap}})",
  "{{percent}}% off on {{module}} (on orders above {{min}})":
    "{{percent}}% off on {{module}} (on orders above {{min}})",
  "{{percent}}% off on {{module}}": "{{percent}}% off on {{module}}",
  "Get {{percent}}% off (up to {{cap}}) (on orders above {{min}})":
    "Get {{percent}}% off (up to {{cap}}) (on orders above {{min}})",
  "Get {{percent}}% off (up to {{cap}})":
    "Get {{percent}}% off (up to {{cap}})",
  "Get {{percent}}% off (on orders above {{min}})":
    "Get {{percent}}% off (on orders above {{min}})",
  "Get {{percent}}% off": "Get {{percent}}% off",
  "Get {{percent}}% off on all orders (up to {{cap}})":
    "Get {{percent}}% off on all orders (up to {{cap}})",
  "Get {{percent}}% off on all orders": "Get {{percent}}% off on all orders",
  "Free delivery on {{module}} (on orders above {{min}})":
    "Free delivery on {{module}} (on orders above {{min}})",
  "Free delivery on {{module}}": "Free delivery on {{module}}",
  "{{percent}}% off on delivery fee on {{module}} (on orders above {{min}})":
    "{{percent}}% off on delivery fee on {{module}} (on orders above {{min}})",
  "{{percent}}% off on delivery fee on {{module}}":
    "{{percent}}% off on delivery fee on {{module}}",
  "Delivery fee benefit on {{module}} (on orders above {{min}})":
    "Delivery fee benefit on {{module}} (on orders above {{min}})",
  "Delivery fee benefit on {{module}}": "Delivery fee benefit on {{module}}",
  "Free delivery (on orders above {{min}})":
    "Free delivery (on orders above {{min}})",
  "{{percent}}% off on delivery fee (on orders above {{min}})":
    "{{percent}}% off on delivery fee (on orders above {{min}})",
  "{{percent}}% off on delivery fee": "{{percent}}% off on delivery fee",
  "Delivery fee benefit (on orders above {{min}})":
    "Delivery fee benefit (on orders above {{min}})",
  "Delivery fee benefit": "Delivery fee benefit",
  "Get {{percent}}% off delivery fee": "Get {{percent}}% off delivery fee",
  "Grand Total": "Grand Total",
	"No category found": "No category found",
	"No options found": "No options found",
	"Search category...": "Search category...",
	"Search sub category...": "Search sub category..."

};
