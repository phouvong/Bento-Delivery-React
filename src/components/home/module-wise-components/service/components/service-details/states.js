...
export const reducer = (state, action) => {
    switch (action.type) {
        ...
        case "incrementQuantity": {
            const unitPrice = state.modalData[0]?.selectedOption?.length > 0
                ? state.modalData[0]?.selectedOption?.[0]?.price
                : (state.modalData[0].price ?? state.modalData[0].base_price);
            return {
                ...state,
                modalData: [
                    {
                        ...state.modalData[0],
                        totalPrice: unitPrice * (state.modalData[0].quantity + 1),
