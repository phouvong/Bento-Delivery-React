import { Box } from "@mui/system";
import TabbedSectionPage from "components/home/section-page/TabbedSectionPage";
import isVerifiedStoreEnabled from "helper-functions/isVerifiedStoreEnabled";

export const SECTION_GAP = 3;

export const getServiceSections = (configData) =>
  [
  {
    id: "available-now",
    label: "Available Now",
    icon: (
      <Box sx={{ color: "warning.main", display: "flex" }}>
        <i
          className="fi fi-rr-bolt"
          style={{ fontSize: "16px", lineHeight: 1, display: "flex" }}
        />
      </Box>
    ),
    content: <TabbedSectionPage sectionType="available-now" defaultTab="stores" />,
    mobileContent: <TabbedSectionPage sectionType="available-now" defaultTab="stores" />,
  },
  {
    id: "verified-seller",
    label: "Verified Only",
    icon: (
      <Box sx={{ color: "info.main", display: "flex" }}>
        <i
          className="fi fi-rr-shield-trust"
          style={{ fontSize: "16px", lineHeight: 1, display: "flex" }}
        />
      </Box>
    ),
    content: <TabbedSectionPage sectionType="verified-seller" />,
    mobileContent: <TabbedSectionPage sectionType="verified-seller" />,
  },
  {
    id: "top-rated",
    label: "Top Rated",
    icon: (
      <Box sx={{ color: "warning.dark", display: "flex" }}>
        <i
          className="fi fi-rr-star"
          style={{ fontSize: "16px", lineHeight: 1, display: "flex" }}
        />
      </Box>
    ),
    content: <TabbedSectionPage sectionType="top-rated" />,
    mobileContent: <TabbedSectionPage sectionType="top-rated" />,
  },
  {
    id: "nearby",
    label: "Nearby",
    icon: (
      <Box sx={{ color: "primary.dark", display: "flex" }}>
        <i
          className="fi fi-rs-marker"
          style={{ fontSize: "16px", lineHeight: 1, display: "flex" }}
        />
      </Box>
    ),
    content: <TabbedSectionPage sectionType="nearby" />,
    mobileContent: <TabbedSectionPage sectionType="nearby" />,
  },
  ].filter(
    (section) =>
      section.id !== "verified-seller" || isVerifiedStoreEnabled(configData),
  );
