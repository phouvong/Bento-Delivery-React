...
const PHARMACY_BANNER_TITLES = {
  Offers: "Search Medicine Offers",
  "Free Delivery": "Find Free Delivery Medicine",
  "Top Rated": "Search Top Rated Pharmacies",
  Nearby: "Search Nearby Pharmacies",
  "Shop By Conditions": "Search by Health Condition",
};

const PharmacySectionPageWrapper = ({ sectionLabel, enableSearch = true, childHandlesSearch = false, children }) => {
  const { t } = useTranslation();
  const router = useRouter();

  const q = enableSearch && typeof router.query.q === "string" ? router.query.q.trim() : "";
  ...
  const filterButton = enableSearch ? (
    ...
    >
      <i className="fi fi-ss-sliders-v" style={{ fontSize: "16px", lineHeight: 1, display: "flex" }} />
      {appliedFilterCount > 0 && (
        <Box sx={{ position: "absolute", top: -8, right: -8, minWidth: 18, height: 18, borderRadius: "50%", backgroundColor: "neutral.1050", display: "flex", alignItems: "center", justifyContent: "center", px: "4px" }}>
          <Typography sx={{ fontSize: "11px", fontWeight: 700, color: "#fff", lineHeight: 1 }}>{appliedFilterCount}</Typography>
        </Box>
      )}
    </Box>
  ) : undefined;
  ...
  return (
    <>
      ...
      <ModuleSectionPageWrapper
        ...
        bannerTitle={PHARMACY_BANNER_TITLES[sectionLabel] ?? "Search Medicines"}
        bannerSubtitle="Easy search to easy order & get fast delivery for your need."
        searchComponent={enableSearch ? <SectionSearchBar placeholder={t("Search Here...")} /> : undefined}
      ...
      {enableSearch && (
        <FoodSearchFilterDrawer anchorEl={filterAnchorEl} onClose={() => setFilterAnchorEl(null)} onApply={handleFilterApply} filterValue={filterParams} showFilterBy={true} categories={categories} />
      )}
    </>
  );
};

export default PharmacySectionPageWrapper;
