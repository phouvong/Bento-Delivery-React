...
const MyTrips = ({
  ...
  const displayTrips = ordersData?.orders ?? [];
  const totalSize = ordersData?.total_size;

  const groupByDay = (trips) => {
    const groups = [];
    const map = new Map();
    (trips ?? []).forEach((trip) => {
      const day = trip?.created_at
        ? new Date(trip.created_at).toDateString()
        : "Unknown";
      if (!map.has(day)) {
        map.set(day, []);
        groups.push({ day, trips: map.get(day) });
      }
      ...
      <CustomStackFullWidth spacing={3} sx={{ pt: "16px" }}>
        {isLoadingOrder ? (
          ...
          <Stack spacing={{ xs: "16px", md: "32px" }}>
            {groupByDay(displayTrips).map(({ day, trips: dayTrips }) => (
              <Stack key={day} spacing={2}>
                ...
                >
                  {dayTrips.map((trip) => (
                    ...
                    >
                      <Trip order={trip} t={t} configData={configData} />
                    </Box>
                  ))}
                </Stack>
              </Stack>
