...
const SEO = ({
  ...
  const buildRobotsContent = () => {
    ...
    if (robotsMeta.meta_no_image_index)
      robotsArray.push(robotsMeta.meta_no_image_index);
    if (robotsMeta.meta_no_archive)
      robotsArray.push(robotsMeta.meta_no_archive);
    if (robotsMeta.meta_no_snippet)
      robotsArray.push(robotsMeta.meta_no_snippet);
    ...
    if (
      robotsMeta.meta_max_video_preview &&
      robotsMeta.meta_max_video_preview_value
    ) {
      robotsArray.push(
        `max-video-preview:${robotsMeta.meta_max_video_preview_value}`,
      );
    }

    // Handle max-image-preview
    if (
      robotsMeta.meta_max_image_preview &&
      robotsMeta.meta_max_image_preview_value
    ) {
      robotsArray.push(
        `max-image-preview:${robotsMeta.meta_max_image_preview_value}`,
      );
    }
    ...
    <>
      ...
      <Head>
        ...
        {metaImage ? (
          <meta
            itemProp="image"
            content={metaImage || configData?.logo_full_url}
          />
        ) : (
          <meta itemProp="image" content={configData?.logo_full_url} />
        )}
