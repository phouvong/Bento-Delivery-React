...
const DeliveryInstruction = (props) => {
  ...
  const applyInstruction = () => {
    ...
    if (selectedInstruction) {
      finalInstruction =
        customNote !== ""
          ? `${selectedInstruction} ( ${customNote} )`
          : `${selectedInstruction}`;
    } else {
      finalInstruction = `${customNote}`;
    }
    setCustomerInstruction(finalInstruction);
    // Persist the applied value so parent state reflects it after
    // close — caller can read selectedInstruction without re-opening.
    setSelectedInstruction(finalInstruction);
    // Keep the local textarea state in sync with what was just applied
    // so reopening the modal shows the committed value.
    setValue(finalInstruction);
    setOpenModal(false);
  };
      ...
      {deliveryInstruction?.data.length > 0 ? (
        <>
          <Typography
            ...
            variant="h6"
          >
            {t("Choose Delivery Instructions")}
          </Typography>
                    ...
                    <FormControlLabel
                      ...
                      sx={{
                        ...
                        "& .MuiFormControlLabel-label": {
                          color:
                            selectedInstruction === instruction.instruction
      ...
      {error && (
        <Stack flexDirection="row" alignItems="center" gap="5px">
          ...
          <Typography color={theme.palette.error.main}>
            {t("Note must not be grater than 191 characters")}
          </Typography>
        </Stack>
      )}
      <Button
