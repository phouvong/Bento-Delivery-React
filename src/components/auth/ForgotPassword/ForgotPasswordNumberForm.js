...
const ForgotPasswordNumberForm = ({
  ...
  const handleOnChange = (value) => {
    // Empty input must clear the field — otherwise prefixing "+" to an
    // empty string leaves a lone "+" hanging in the form value.
    if (!value) {
      phoneFormik.setFieldValue("phone", "");
      return;
    }
  ...
  const handleClick = () => {
    // Close the forgot-password modal and ask the navbar to reopen the
    // sign-in modal directly into SignInForm. Two separate modals can't
    // both render here (the nested one would unmount with us), so the
    // navbar owns the next modal via the `openSignInModal` flag.
