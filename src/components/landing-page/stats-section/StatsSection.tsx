...
const StatsSection: React.FC<StatsSectionProps> = ({ trustSectionData }) => {
  ...
  return (
    <CustomContainer maxWidth="lg" noMobilePadding={false} mobilePx={undefined}>
      <Grid
        container
        spacing={{ xs: 1, sm: 3 }}
        pt={{ xs: "1rem", md: "3.5rem" }}
      >
        {activeCards.map((card: TrustCard, index: number) => (
          <Grid item xs={6} sm={6} md={3} key={index}>
            <StatsCard>
              ...
              <ContentContainer>
                <StatsNumber>{card.title}</StatsNumber>
                {card.sub_title && (
                  <StatsTitle variant="body2">{card.sub_title}</StatsTitle>
                )}
              </ContentContainer>
            </StatsCard>
          </Grid>
